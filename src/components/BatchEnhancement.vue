<template>
      ...
      <div class="batch-enhancement">
        ...
        <div class="footer">
          ...
          <!-- 按钮 -->
          <div :class="['btn-primary', batchEnhancement.length ? null : 'disable-btn']" @click="submitBatchEnhancement">{{$t('MOMO_101')}}</div>
        </div>
      </div>
    </Dialog>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async submitBatchEnhancement() {
        ...
        const isMomoverse = this.batchEnhancement[0].location == ConstantConfig.NFT_LOCATION.VERSE;

        await Wallet.ETH.Group.Enhancer.batchEnhance(tokenIds, hrUps, crystals, isMomoverse);
...
<style lang="less" scoped>
  ...
  .list {
    ...
    background: #13181f;
    border-radius: 16px;
    box-shadow: 2px 2px 2px 0px rgba(26, 32, 38, 0.40) inset, 0px 0px 4px 0px rgb(46, 68, 87) inset;

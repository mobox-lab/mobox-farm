<template>
  <li :class="['pet_hover_lv' + item.vType, 'item-layout']">
    ...
    <div class="info-box">
      <!-- momo图片 -->
      <div class="image">
        <img class="pet_img" v-if="item.prototype > 60000" :src="require(`@/assets/pet/${item.prototype}.gif`)" alt=""  height="120" />
        <img class="pet_img" v-else :src="require(`@/assets/pet/${item.prototype}.png`)" alt=""  height="120" />
      </div>
      <!-- 算力信息 -->
      <div class="info">
        <div class="item">
    ...
    <div class="operation">
      ...
      <!-- 升级按钮 -->
      <div class="form">
        <SelectNum :needHideReduce="false" :maxNum="getMaxUpgrade(item)" :defaultNum="count" :onChange="upgradeNumberChange" />
      </div>
    </div>
  </li>
</template>
  ...
  export default {
    ...
    computed: {
      ...
      upgradeHashPower() {
        ...
        const newHashrate = +hashrate + this.count;
        return parseInt(newHashrate + parseInt(growup.staticPower * (level - 1) /100) + parseInt(((parseInt(level / 5) * (1 + parseInt(level / 5)))) * growup.staticPower * growup.staticPercent / 200));
      },
      // 消耗的水晶
      needCrystal(){

    ...
    <div ref="container" class="swiper-container dib" :id="data.item.tx">
      <div class="swiper-wrapper">
        <div class="swiper-slide" v-for="item in getShowList" :key="item.id">
          <div
            class="pet-lv vertical-children"
            v-if="Number(item.imgName) > 100"
          >
            <img
              :src="
                require(`../assets/icon/${item.chain.toLocaleLowerCase()}.png`)
              "
              alt=""
              width="15"
            />&nbsp;
          ...
          <div style="position: absolute; width: 100%; bottom: 50px; left: 0px">
            <div class="vertical-children pet_num">x{{ item.num }}</div>
          </div>
          <div
            style="position: absolute; width: 100%; top: -10px; left: 0px"
            v-if="item.erc1155_ != 1 && item.currency == 3"
          >
            <div class="vertical-children pet_num small">
              <span
                >{{ $t("Market_39") }}:
                {{ numFloor(item.price / item.num / 1e9, 100) }}
                {{ getCurrencyName(item.currency) }}</span
              >
            </div>
          </div>
        </div>

<template>
	...
	<div class="mgb-20" v-if="getNowPetItem.vType >= 4">
		<h3>
			{{$t("MOMO_57")}}
			<img class="mgl-5 cur-point" @click="oprDialog('enhance-rule-dialog','block')" src="@/assets/icon/help.png" alt="" height="20"/>
		</h3> 
		...
		<div v-else class="ly-input-content mgt-10" style="padding-bottom:20px">
			...
			<div class="mgt-10 dib"  style="background:rgba(0,0,0,0.3);border-radius: 15px;padding:12px;font-size:18px; ">
				<button class="btn-primary btn-small" @click="enhanceTimes = 1">Min</button>&nbsp;
				<SelectNum id="enhance-num-select" :needHideReduce="false" :maxNum="getMaxEnhanceTimes"  :defaultNum="enhanceTimes"  :onChange="onNumChange" />&nbsp;
				<button class="btn-primary btn-small mgl-5" @click="enhanceTimes = getMaxEnhanceTimes">Max</button> 
			</div>

			<p class="small opa-6 mgt-20">{{$t("MOMO_73")}}</p>
			<div class=" mgt-10" style="background:rgba(0,0,0,0.3);border-radius: 15px;padding:12px;font-size:18px; display:flex">
			...
			<div class="ovh mgt-10">
				<div class="col-md-6 col-xs-12">
					...
					<div class="mgt-10" style="font-size:18px">
						...
						<span style="color: #85f34a">{{ getNowPetItem.hashrate +  Number(enhanceTimes)}}</span>
					</div>
				</div>
				<div class="col-md-6  col-xs-12">
					<template v-if="getNowPetItem.level > 1">
					<p class="small opa-6 mgt-10">{{$t("MOMO_61")}}(LV.{{getNowPetItem.level}})</p>
					<div class="mgt-10"  style="font-size:18px">
						<img src="@/assets/icon/airdrop.png" height="25" alt="" />&nbsp;
						<span>{{ getNowPetItem.lvHashrate }}</span>
						<img src="@/assets/icon/upgradejt.png" alt="" class="mgl-10" />
						<img src="@/assets/icon/airdrop.png" alt="" class="mgl-10" height="25" />&nbsp;
						<span style="color: #85f34a">{{ getTargetLevelHashPower(getNowPetItem.hashrate +  Number(enhanceTimes)) }}</span>
					</div>
				</template>
				</div>
			</div>
			<div class="mgt-30 tac button-layout">
				<div :class="{'btn-group': needApproveMec}">
					...
					<div class="mgt-10">
						<StatuButton data-step="2" :onClick="enhance" style="min-width:150px" :isDisable="needApproveMec" :isLoading="lockBtn.enhanceLock > 0">
							{{$t("MOMO_65")}}
						</StatuButton>
					</div>
				</div>
				<!-- 批量进化 -->
				<div class="btn-primary enhancement mgt-10" @click="toggleBatch">{{$t(isExistsBatch ? 'MOMO_104' : 'MOMO_103')}}</div>
			</div>
		</div>
...
export default {
	...
	computed: {
		...
		isExistsBatch() {
			return this.batchEnhancement.some(item => item.tokenId == this.data.tokenId);
		},
		getMaxEnhanceTimes(){
		...
		isMaxEnhance(){
			let {vType, hashrate} = this.getNowPetItem;
			return (vType == 4 && hashrate == this.momoSetting.v4_max_enhance) 
		...
		getRulerCfg(){
			...
			return Object.values(BaseConfig[`MomoEnhanceV${vType}Cfg`]).filter(item=>item.hashrate < Number(this.momoSetting[`v${vType}_max_enhance`]));
		}
	},
	async created(){
	...
	methods: {
		...
		onNumChange(data, num){
			...
			if(this.enhanceTimes >= this.getMaxEnhanceTimes ) this.enhanceTimes =  this.getMaxEnhanceTimes;
		},
		...
		async enhance(){
			...
			let hash = await Wallet.ETH.Group.Enhancer.enhanceV2(data, ()=>{
				this.enhanceTimes = 1;
		...
		async viewMECApproved(){
			let isApprove = await Wallet.ETH.isApprovedForAll(WalletConfig.ETH.crystalToken, WalletConfig.ETH.moMoToken);
			if(isApprove != null){
		...
		async approveMEC(){
			let hash = await Wallet.ETH.approvedForAll(WalletConfig.ETH.crystalToken, WalletConfig.ETH.moMoToken, ()=>{

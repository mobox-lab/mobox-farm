<template>
  <div>
    <MomoEnhance
      :data="this.data"
      v-if="!this.isMarket && getNowPetItem.location != 'wallet'"
    />
    ...
    <div v-if="getNowPetItem.vType >= 4 && getNowPetItem.gems" class="mgb-20">
      <h3>
        {{
          getNowPetItem.location != "wallet" && !isMarket
            ? $t("MOMO_40")
            : $t("Gemstone_44")
        }}
      </h3>
      <div class="ly-input-content mgt-10" style="padding-bottom: 20px">
        <div class="aveage-box tac mgt-10" id="gem-show">
          <div
            v-for="(item, index) in [100, 200, 300, 400]"
            :key="item"
            class="tac"
          >
            <div
              ...
              @click="showGemBag(index)"
            >
              <img
                :class="{ 'opa-3': Number(data.gems[index]) == 0 }"
                :src="
                  require(`@/assets/market/${
                    data.gems[index] == 0 ? item + 1 : data.gems[index]
                  }.png`)
                "
                alt=""
                width="100%"
              ...
              <template v-if="getNowPetItem.location != 'wallet' && !isMarket">
                ...
                <template v-else>
                  <svg
                    v-if="campObj[index]"
                    :class="{ gray: isDisableOpr }"
                    @click.stop="
                      gemWearPos = index;
                      oprDialog('confirm-upgrade-dialog', 'block');
                    "
                    style="
                      position: absolute;
                      right: 3px;
                  ...
                  >
                    <path
                      fill="#E1FF17"
                      d="M583.586909 555.473455a62.091636 62.091636 0 0 0 0-86.94691L151.970909 30.138182a60.113455 60.113455 0 0 0-85.783273 0 62.045091 62.045091 0 0 0 0 86.946909L454.981818 512 66.210909 906.938182a62.045091 62.045091 0 0 0 0 86.923636 60.043636 60.043636 0 0 0 85.783273 0l431.592727-438.388363zM440.459636 117.061818L829.160727 512 440.413091 906.938182a62.045091 62.045091 0 0 0 0 86.923636 60.090182 60.090182 0 0 0 85.806545 0l431.569455-438.388363a62.091636 62.091636 0 0 0 0-86.94691L526.196364 30.138182a60.136727 60.136727 0 0 0-85.806546 0 62.045091 62.045091 0 0 0 0.023273 86.923636z"
                    ></path>
                  </svg>
                  <Loading
                    v-if="lockBtn.upgradeGemWearLock > 0"
                    style="position: absolute; right: 3px; top: 3px"
                    :width="20"
                    :height="20"
                  />
            ...
            <div
              class="tac mgt-10"
              style="height: 30px"
              v-if="getNowPetItem.location != 'wallet' && !isMarket"
            >
              <StatuButton
                :onClick="removeGem.bind(this, index)"
                v-if="Number(data.gems[index]) != 0"
                :isDisable="isDisableOpr || data.rent.state != -1"
                :isLoading="lockBtn.takeOffGemLock > 0"
                class="btn-small"
                style="width: 100%; max-width: 80px"
              >
                <img
                  src="@/assets/icon/rent_time.png"
                  alt=""
                  height="15"
                  v-if="data.rent.state != -1"
                />
        ...
        >
          <div class="gemBag" @click="oprDialog('gemBag-dialog', 'block')">
            ...
            <p class="stroke" :data-text="$t('Gemstone_16')">
              {{ $t("Gemstone_16") }}
            </p>
          </div>
        </div>
    ...
    <div v-if="!isMarket" class="mgb-20">
      ...
      <p class="mgt-10 small opa-6">
        <span v-if="getNowPetItem.location == 'stake'">{{
          $t("MOMO_42")
        }}</span>
        <span v-if="getNowPetItem.location == 'wallet'">{{
          $t("MOMO_43")
        }}</span>
        <span v-if="getNowPetItem.location == 'verse'"
          >该momo在MoMoverse中挖矿</span
        >
      </p>
      <div class="tac mgt-20">
        <div
          v-if="getNowPetItem.location == 'wallet'"
          :class="needApprove ? 'btn-group' : ''"
        >
          <div v-if="needApprove">
            <button
              data-step="1"
              :class="
                needApprove && lockBtn.approveLock <= 0 ? '' : 'disable-btn'
              "
              style="width: 200px"
              class="btn-primary por"
              @click="
                needApprove && lockBtn.approveLock <= 0 && approveNftToStake()
              "
            >
              <Loading
                class="btn-loading"
          ...
          <div class="mgt-10">
            <button
              data-step="2"
              :class="
                canShowStake && lockBtn.stakeLock <= 0 ? '' : 'disable-btn'
              "
              style="width: 200px"
              class="btn-primary por"
              @click="canShowStake && lockBtn.stakeLock <= 0 && stakeNft()"
            >
              <Loading
                class="btn-loading"
                :width="15"
                :height="15"
                v-if="lockBtn.stakeLock > 0"
              />{{ $t("MOMO_44") }}
            </button>
          </div>
        </div>
        ...
        <div v-if="getNowPetItem.location == 'stake'">
          <button
            :class="lockBtn.unStakeLock <= 0 ? '' : 'disable-btn'"
            class="btn-primary por"
            @click="cancelApply"
          >
            <Loading
    ...
    <!-- 升级记录｜进化记录 -->
    <div
      v-if="levelUpInfo.length > 0 || enhanceHistory.length > 0"
      class="mgb-20 ly-input-content"
    >
      <div class="por">
        <h3
          class="dib cur-point"
          :class="{ tabActive: historyTab == 0 }"
          @click="historyTab = 0"
        >
          {{ $t("MOMO_34") }}
        </h3>
        <h3
          class="dib cur-point mgl-10"
          :class="{ tabActive: historyTab == 1 }"
          @click="historyTab = 1"
        >
          {{ $t("MOMO_67") }}
        </h3>
      ...
      >
        ...
        >
          ...
          <div class="tac">
            <p
              class="vertical-children tac mgt-10 small dib"
              style="
            ...
            >
              ...
              <span class="c-lv1">{{
                $t("MOMO_74").replace(
                  "#0#",
                  item.newHashrate - item.oldHashrate
                )
              }}</span>
            </p>
          </div>
    ...
    <div v-if="tradeHistory.length > 0" class="mgb-20">
      ...
      <div style="max-height: 300px; overflow-y: auto" class="mgt-10">
        <div class="history-item" v-for="item in tradeHistory" :key="item.tx">
          ...
          <div
            class="row tal cur-point por"
            style="background: #1c222c; border-radius: 6px; padding: 5px"
          >
            <table class="pj-table">
              <tr style="color: #fff">
                ...
                <td class="tar">
                  ...
                  <p style="color: #fff; zoom: 0.8" class="opa-6">
                    {{
                      dateFtt(
                        "yyyy-MM-dd hh:mm:ss",
    ...
    <!-- 故事记录 -->
    <div
      v-if="hasStorySkill && getNowPetItem.location == 'stake'"
      class="mgb-20"
    >
      <div v-if="!isMarket">
        <h3>{{ $t("MOMO_24") }}</h3>
        <div class="mgt-10">
          <textarea
            ...
            cols="30"
            rows="10"
            :placeholder="$t('MOMO_38')"
            maxlength="256"
          ></textarea>
        </div>
        <span class="small mgt-10 opa-6"
          >{{ $t("MOMO_25") }}({{ nowLen }}/256)</span
        >
        <div class="tac">
          <button
            class="btn-primary"
            :class="inputStory == '' ? 'disable-btn' : ''"
            @click="submitStory"
          >
            {{ $t("Common_03") }}
          </button>
      ...
      <h3 v-if="storyList.length > 0" class="mgt-20 por">
        {{ $t("MOMO_26") }}
      ...
      <div class="mgt-10" style="max-height: 200px; overflow: auto">
        <div class="pet-des-item" v-for="item in storyList" :key="item.tx">
          ...
          <div class="mgt-10" style="word-break: break-all">
            {{ item.storyInfo }}
          </div>
        </div>
      </div>
    ...
    <Dialog id="unstake-confirm-dialog" :top="180" :width="350">
      ...
      <div class="mgt-50">
        <button
          class="btn-default"
          @click="oprDialog('unstake-confirm-dialog', 'none')"
        >
          {{ $t("Common_04") }}
        </button>
        <button class="btn-primary mgl-10" @click="unStakeNftConfirm()">
          {{ $t("Common_03") }}
        </button>
      </div>
    </Dialog>
    ...
    <Dialog id="gem-wear-dialog" :top="180" :width="450">
      <div class="ly-input-content por">
        <div class="aveage-box" v-for="item in [0, 4, 8]" :key="item">
          <div
            v-for="item2 in [1, 2, 3, 4]"
            :key="item + item2"
            style="padding: 5px"
          >
            <div
              v-if="item + item2 <= 9"
              class="gem-item gem-border"
              :class="[
                { active: selectGemLv == item + item2 },
                'border_' + selectGemType,
                {
                  'opa-6': Number(gemBag[selectGemType + (item + item2)]) <= 0,
                },
              ]"
              @click="selectGemLv = item + item2"
            >
              <img
                :src="
                  require(`@/assets/market/${selectGemType + item + item2}.png`)
                "
                alt=""
                width="100%"
              />
              <span class="gem-num stroke bold">{{
                gemBag[selectGemType + (item + item2)]
              }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="ly-input-content mgt-10">
        ...
        <div
          class="mgt-10"
          :class="{ 'btn-group': !hasApprove && hasApprove != -1 }"
          v-if="selectGemLv > 0"
        >
          <StatuButton
            v-if="hasApprove != -1 && !hasApprove"
            data-step="1"
          ...
          <StatuButton
            data-step="2"
            class="mgt-10"
            :onClick="wearGem.bind(this)"
            style="width: 60%"
            :isDisable="
              !hasApprove ||
              Number(gemBag[selectGemType + selectGemLv]) <= 0 ||
              isDisableOpr ||
              hasSameGem ||
              hasRentAndLowGem
            "
            :isLoading="lockBtn.wearGemLock > 0"
          >
            <img
              src="@/assets/icon/rent_time.png"
              alt=""
              height="15"
              v-if="hasRentAndLowGem"
            />
            <span v-else>{{
    ...
    <Dialog id="confirm-upgrade-dialog" :top="200" :width="350">
      <h4
        class="mgt-30"
        v-html="
          $t('Gemstone_40').replace(
            '#0#',
            `<span style='color: #49c773'>LV.${getNextLv}</span>`
          )
        "
      ></h4>
      <div class="mgt-50">
        <button
          class="btn-primary"
          @click="oprDialog('confirm-upgrade-dialog', 'none')"
        >
          {{ $t("Common_04") }}
        </button>
        <button
          ...
          @click="
            oprDialog('confirm-upgrade-dialog', 'none');
            upgradeGemWear();
          "
        >
          {{ $t("Common_03") }}
...
export default {
  ...
  computed: {
    ...mapState({
      allowance_1155_to_stake: (state) =>
        state.ethState.data.allowance_1155_to_stake,
      allowance_721_to_stake: (state) =>
        state.ethState.data.allowance_721_to_stake,
    ...
    isDisableOpr() {
      return (
        this.lockBtn.wearGemLock > 0 ||
        this.lockBtn.takeOffGemLock > 0 ||
        this.lockBtn.upgradeGemWearLock > 0
      );
    },
    //
    hasSameGem() {
      return (
        Number(this.getNowPetItem.gems[this.gemWearPos]) % 100 ==
        this.selectGemLv
      );
    },
    //
    ...
    campObj() {
      ...
      this.getNowPetItem.gems.map((item, index) => {
        ...
        if (gemLv > 0 && gemLv < ConstantConfig.NOW_MAX_GEM_LEVEL) {
          canCompArr[index] =
            Number(this.gemBag[this.gemPidToPos[index] + gemLv]) >= 2;
        }
      });
      return canCompArr;
    },
  ...
  methods: {
    ...
    async upgradeGemWear() {
      ...
      let hash = await Wallet.ETH.inlayQuickLvUp({
    ...
    async removeGem(pos_) {
      let hash = await Wallet.ETH.takeOffGem({
        momoId_: this.getNowPetItem.tokenId,
        pos_,
      });
      if (hash) {
    ...
    async approveNftToStake() {
      ...
      let hash;
      if (vType < 4) {
        hash = await Wallet.ETH.approve1155ToTargetToken(
          WalletConfig.ETH.moMoStake
        );
      } else {
        hash = await Wallet.ETH.approve721ToTargetToken(
          WalletConfig.ETH.moMoStake
        );
      }
      if (hash) {
    ...
    async stakeNft() {
      ...
      let hash = await Wallet.ETH.stakeNft(
        erc1155ids,
        erc1155Num,
        erc721TokenIds
      );
    ...
    async unStakeNftConfirm() {
      ...
      let hash = await Wallet.ETH.unStakeNft(
        erc1155ids,
        erc1155Num,
        erc721TokenIds
      );
    ...
    async submitStory() {
      ...
      if (
        lastEdit &&
        lastEdit.owner.toLocaleLowerCase() == myAddr.toLocaleLowerCase()
      ) {
      ...
      let res = await Wallet.ETH.setMomoStoryByTokenId(
        tokenId,
        this.inputStory,
        storys.length == ""
      );

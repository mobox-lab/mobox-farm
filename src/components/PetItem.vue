<template>
  <div
    :class="[
      'pet_hover_lv' + data.item.vType,
      { market: data.item.location == 'verse' },
    ]"
    class="pet_item"
  ...
  >
    <img
      ...
      alt=""
      @click.stop="quickBuy(data.item.prototype)"
    />
    ...
    <div class="jewel" v-if="data.item.vType >= 4">
      ...
      >
        <img
          v-if="data.item.gems == undefined || data.item.gems[index] == 0"
          :src="require(`../assets/market/${item + 1}.png`)"
          class="opa-3 gray"
          alt=""
          title="jewel"
        />
        <img
          v-else
          :src="require(`../assets/market/${data.item.gems[index]}.png`)"
          alt=""
        />
      </div>
    </div>
    ...
    <img
      class="pet_img"
      v-if="Number(data.item.prototype) > 60000"
      :src="require(`../assets/pet/${data.item.prototype}.gif`)"
      alt=""
      height="120"
    />
    <img
    ...
    >
      <div
        class="vertical-children pet_num"
        v-if="data.item.vType < 4 && data.item.num > 1"
      >
        x{{ data.item.num }}
      </div>
    </div>
    ...
    <div class="pet-bottom vertical-children mgt-5">
      <div class="pet-name">
        <img
          ...
          :src="
            require(`../assets/icon/${category_img[data.item.category]}.png`)
          "
          alt=""
          width="15"
          height="15"
    ...
    <!-- LV.1 -->
    <div
      class="mgt-10 tar vertical-children lv1"
      style="font-size: 12px"
      v-if="
        (data.item.vType >= 4 && data.item.level > 1) ||
        data.item.location == 'verse'
      "
    >
      <img
        v-if="!isMeetStandards && isShowHashrateIcon"
        src="@/assets/icon/warning-icon.png"
        class="tip-icon"
        @click.stop="standardsHashrateTip"
      />&nbsp; <span>Lv. 1</span>&nbsp;
      <img src="../assets/icon/airdrop.png" alt="" height="15" />&nbsp;
      <span :class="getHashrateColor(data.item)" class="bold">{{
        data.item.hashrate
      }}</span>
    </div>
    ...
    <div class="pet-power vertical-children mgt-20">
      <img
        v-if="!isMeetStandards && !(data.item.level > 1) && isShowHashrateIcon"
        src="@/assets/icon/warning-icon.png"
        class="tip-icon"
        @click.stop="standardsHashrateTip"
      />
      <div
        class="gka-harmer por"
        ref="anime"
        style="margin-top: -12px; right: -10px"
        :style="{ animationDelay: delay + 's' }"
        :class="{ 'animation-harmer': data.item.location == 'stake' }"
      >
        <img
          src="../assets/anime/sleep.gif"
          class="sleep-harmer"
          v-if="data.item.location == 'wallet'"
          alt=""
        />
      </div>
      <span
      ...
      >
      <div
        class="mgt-10 tar vertical-children show-only-market hide"
        style="font-size: 12px"
        v-if="data.item.vType >= 4 && data.item.level > 1"
      >
        <img
          v-if="!isMeetStandards && isShowHashrateIcon"
          src="@/assets/icon/warning-icon.png"
    ...
    <div
      class="pet-chanzi vertical-children"
      v-if="data.item.location == 'verse'"
    >
      <img

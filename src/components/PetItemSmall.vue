<template>
  <div
    class="pet-add-item por"
    @click="addNum"
    :class="{ 'opa-6': data.isLock }"
  >
    <div :class="isSelect || data.num <= 0 ? 'opa-4' : ''">
      <div :class="'skew-box v' + data.vType"></div>
      <img
        v-if="getSelectNum > 0 && data.vType < 4"
        class="reduce-btn"
        src="../assets/icon/reduce_pet.png"
        alt=""
        @click="reduceNum($event)"
      />
      <div
        class="pet-item-hashrate vertical-children"
        v-if="data.vType >= 4 && data.lvHashrate"
      >
      ...
      <div
        class="pet-item-lv vertical-children"
        v-if="data.vType >= 4 && data.level"
      >
      ...
      <div class="pet-add-show-img">
        ...
        />
        <img
          :src="require(`../assets/pet/${data.prototype}.png`)"
          alt=""
          width="90%"
        />
      </div>
      <div class="pet-add-type vertical-children">
        <img
          :src="require(`../assets/icon/${category_img[data.category]}.png`)"
          alt=""
          width="15"
          height="15"
        />
    ...
    <span
      class="opa-6"
      v-if="data.num <= 0 || (getSelectNum == data.num && data.vType < 4)"

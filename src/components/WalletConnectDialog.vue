<template>
  <div>
    <Dialog id="connect-wallet-dialog" :top="100" :width="400">
      ...
      <div class="mgt-20">
        <div
          class="connect-wallet-item aveage-box mgt-10"
          @click="connectWallet('mboxWallet')"
        >
          <div class="tal">
          ...
          <div class="tar">
            <img
              ...
              src="../assets/icon/mboxWallet.png"
              height="30"
              alt=""
            />
          </div>
        ...
        >
          ...
          <div class="tar">
            <img
              style="margin-top: 5px"
              src="../assets/icon/trust-wallet.png"
              height="30"
              alt=""
            />
          </div>
        ...
        >
          ...
          <div class="tar">
            <img
              style="margin-top: 5px"
              src="../assets/icon/binanceWallet.svg"
              height="30"
              alt=""
            />
          </div>
    ...
    <Dialog id="connected-wallet-info-dialog" :top="100" :width="400">
      ...
      <div class="mgt-50">
        ...
        <p class="tac small opa-6 mgt-10">
          <a
            :href="'https://bscscan.com/address/' + connectWalletAddr"
            target="_blank"
      ...
      <div class="mgt-50">
        <button class="btn-primary" @click="logoutWallet">
          {{ $t("Common_25") }}
        </button>
      </div>
    </Dialog>
    ...
    async connectWallet(type) {
      ...
      switch (type) {
        case "walletConnect":
          ...
          Common.walletConnectConnector.on(
            "session_update",
            (error, payload) => {
              if (error) {
                throw error;
        ...
        case "bitget":
          ...
          if (typeof _provider !== "undefined") {
            try {
              //
              let accounts = await _provider.request({
                method: "eth_requestAccounts",
              });
              account = accounts[0];
            } catch (error) {
        ...
        case "TrustWallet":
          if (typeof window.ethereum !== "undefined") {
            const mp =
              window.ethereum?.providers?.find((item) => item.isTrustWallet) ||
              window.ethereum;
        ...
        case "binanceChain":
          ...
          if (typeof window.BinanceChain !== "undefined") {
            try {
              //
              let accounts = await window.BinanceChain.request({
                method: "eth_requestAccounts",
              });
              account = accounts[0];
            } catch (error) {
            ...
            //
            let network = await window.BinanceChain.request({
              method: "net_version",
            });
            chainNetwork = network;
            //

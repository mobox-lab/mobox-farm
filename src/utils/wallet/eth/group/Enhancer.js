...
export default class Enhancer {
    ...
    return new Promise((resolve) => {
      ETH.sendMethod(
        ...
        async (data) => {
          ...
          let topic =
            "0x3b47c811ba1d2521c9cd69be0de54739ab4b43cdbe22214bf6dcd2c46a7eeba5";
          let eventAbi = [
            {
              indexed: true,
          ...
          Common.app.$refs.boxBag.showOpenBox(
            "mec",
            ETH.parseEvent(data, topic, eventAbi).crystalAmounts
          );
  ...
  static async addMysteryBox(amount_, recipt) {
    ...
    return new Promise((resolve) => {
      ETH.sendMethod(
        contract.methods.addMysteryBox(amount_),
        { from: myAddr },
        (hash) => resolve(hash),
        async () => {
  ...
  static async applyBuy(fcName, applyNum_, recipt) {
    ...
    return new Promise((resolve) => {
      ETH.sendMethod(
        contract.methods[fcName](applyNum_),
        { from: myAddr },
        (hash) => resolve(hash),
        async () => {
  ...
  //
  static async addCrystal(
    { tokenId_, hrOriginal_, crystalAmount_, mbox_ },
    recipt
  ) {
    let myAddr = await ETH.getAccount(true);
    if (!myAddr) return;
  ...
  static async enhance(tokenId_, recipt) {
    ...
    return new Promise((resolve) => {
      ETH.sendMethod(
        contract.methods.enhance(tokenId_),
        { from: myAddr },
        (hash) => resolve(hash),
        async () => {
  ...
  //
  static async enhanceV2(
    { tokenId_, hrUp_, crystal_ },
    recipt,
    isMoMoVerse = false
  ) {
    let myAddr = await ETH.getAccount(true);
    ...
    return new Promise((resolve) => {
      ETH.sendMethod(
        contract.methods.enhance(tokenId_, hrUp_, crystal_),
        { from: myAddr },
        (hash) => resolve(hash),
        async () => {
          console.log("enhance success");
  ...
  static async batchEnhance(tokenIds, hrUps, crystals, isVerse) {
    ...
    if (address) {
      ...
      return new Promise((resolve) => {
        ETH.sendMethod(
          contract.methods.enhance(tokenIds, hrUps, crystals),
          { from: address },
          resolve,
          () => {
            setTimeout(async () => {
              await Common.app.setMyNftByType(
                ConstantConfig.NFT_LOCATION.VERSE
              );
              await Common.app.setMyNftByType(

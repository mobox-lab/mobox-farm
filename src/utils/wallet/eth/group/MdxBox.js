...
export default class MdxBox {
	...
	static async getMdxBoxNum(){
		...
		return new Promise(resolve => {
			contract.methods.balanceOf(myAddr, 1).call().then(data => {
				resolve(data);
			})
		});
	}
	...
	static async openBox(amount_, recipt){
		...
		return new Promise(resolve => {
			ETH.sendMethod(
				contract.methods.openBox(amount_), {from: myAddr},
				hash=>resolve(hash),
				async (data)=>{
					console.log("openBox success", data);
					Common.app.unLockBtn("openActivityBoxLock");

					let topic = "0xcb36cbe73c71192b22f5002fb14ce1aad6fbf3dbefa788903c5ec22d5edf7612";
					let eventAbi = [
							{ "indexed": false, "internalType": "address", "name": "taker", "type": "address" },
							{ "indexed": false, "internalType": "uint256", "name": "amount", "type": "uint256" },
							{ "indexed": false, "internalType": "uint256[]", "name": "luckyAmounts", "type": "uint256[]" },
					]
		...
		let contract = new ETH.web3.eth.Contract([
			{
				...
				"outputs": [
					{"name": "userNormalTicket", "type":"tuple", "components":[
						{"name": "ticketStartNo","type": "uint128"},
						{"name": "ticketEndNo","type": "uint128"},
					]},
					{"name": "userHighTicket", "type":"tuple", "components":[
						{"name": "ticketStartNo","type": "uint128"},
						{"name": "ticketEndNo","type": "uint128"},
					]},
	...
	static async hApplyForBox(applyNum_, recipt){
		...
		return new Promise(resolve => {
			ETH.sendMethod(
				contract.methods.hApplyForBox(applyNum_), {from: myAddr},
				hash=>resolve(hash),
				async ()=>{

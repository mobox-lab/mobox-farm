<template>
	<Dialog id="box-apply-dialog" :top="100" :width="400">
		<div class="tal mgt-10">
			<div @click="dialog_tab_pos =  0;inputNum = '' "  :class="dialog_tab_pos == 0?'active':''" class="tab-menu " >{{$t("NewBOX_25")}}</div>
			<div @click="dialog_tab_pos =  1; inputNum = ''"  :class="dialog_tab_pos == 1?'active':''"  class="tab-menu"  >{{$t("NewBOX_26")}}</div>
		</div>
		<div class="ly-input-content por">
			...
			<div class="mgt-10 aveage-box small">
				...
				<p class="vertical-children tar">
					<span>{{$t("NewBOX_07")}}: {{numFloor(applyInfo.roundPrice / 1e18, 1e4)}}</span>&nbsp;
		...
		<div class="mgt-30">
			...
			<template v-if="getCanUseTemMbox > 0">
				<div class="aveage-box  ly-input-content mgt-10">
					...
					<div class="tar">
						<StatuButton style="width:50%" :onClick="takeMbox" :isLoading="lockBtn.takeMboxLock > 0" v-if="getHighApplyNum + getNormalApplyNum == 0" class="btn-small" :isDisable="Number(myApplyInfo.frozenBalance) <= 0">{{$t("NewBOX_10")}}</StatuButton>
						<button v-else class="btn-primary disable-btn btn-small">{{$t("NewBOX_09")}}</button>
					</div>
				</div>
			</template>
		</div>
		...
		<div class="mgt-50">
			<p v-if="!isStartApply">{{$t("NewBOX_21")}}<span class="dotting"></span></p>
			<p v-if="isHightAndIsMax && isStartApply">{{$t("NewBOX_24")}}</p>
			<div class="tac" v-if="Number(inputNum) > 0 && isCanApply">
				<p v-if="getCanUseTemMbox > 0">{{$t("NewBOX_11").replace("#0#",getCanUseTemMbox > getNeedPayMbox? numFloor(getNeedPayMbox, 1e4): numFloor(getCanUseTemMbox, 1e4))}}</p>

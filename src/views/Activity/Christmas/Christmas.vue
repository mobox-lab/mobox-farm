  ...
  <div class="tac">
    <section class="top" style="overflow: hidden">
      ...
      <div class="por" style="z-index: 10">
        <img
          src="@/assets/christmas/top_title.png"
          alt=""
          class="title-img"
          width="30%"
        />
        <div class="vertical-children">
          <p
            id="title-des"
            class="stroke2 bold dib"
            :data-text="$t('Festival_48')"
          >
            {{ $t("Festival_48") }}
          </p>
          <img
            class="mgl-10 cur-point help"
            @click="getRootRefs().ruleDialog.show('Festival_40', 'Festival_41')"
        ...
        <p class="c-time vertical-children mgt-50 bold" v-if="endTs > nowTs">
          ...
          <span class="mgl-20">{{
            getLeftTime2(endTs - nowTs).split(":")[1]
          }}</span>
          : <span>{{ getLeftTime2(endTs - nowTs).split(":")[2] }}</span> :
          <span>{{ getLeftTime2(endTs - nowTs).split(":")[3] }}</span>
        </p>
        <h2 v-else class="mgt-50 stroke2 bold" style="height: 80px">
          {{ $t("Festival_54") }}
        </h2>
        <div class="por">
          <img
            class="shou"
            v-if="nowTs % 10 < 2"
            src="@/assets/christmas/shou.gif"
            alt=""
            width="30%"
          />
          <img
            style="display: block"
            src="@/assets/christmas/center_t.png"
            alt=""
            width="100%"
          />
        </div>
    ...
    <section class="center por" style="z-index: 11">
      <img
        class="hide-xs"
        style="position: absolute; top: 260px; left: 150px"
        src="@/assets/christmas/wz.png"
        alt=""
        width="80"
      />
      <img
      ...
      <div class="data-view">
        <div class="aveage-box">
          <div class="data-view-item">
            <p>{{ $t("Festival_42") }}</p>
            <h1
              class="stroke3 mgt-10"
              :data-text="numFloor(ovData[0].amount, 100)"
            >
              {{ numFloor(ovData[0].amount, 100) }}
            </h1>
          </div>
          <div class="data-view-item">
            <p>{{ $t("Festival_43") }}</p>
            <h1
              class="stroke3 mgt-10"
              :data-text="numFloor(ovData[1].amount, 100)"
            >
              {{ numFloor(ovData[1].amount, 100) }}
            </h1>
          </div>
        </div>
        ...
        <div class="aveage-box">
          <div class="data-view-item">
            ...
            <h1
              class="stroke3 mgt-10"
              :data-text="numFloor(ovData[2].amount, 100)"
            >
              {{ numFloor(ovData[2].amount, 100) }}
            </h1>
          </div>
          <div class="data-view-item">
            <p>{{ $t("Festival_45") }}</p>
            <h1
              class="stroke3 mgt-10"
              :data-text="numFloor(ovData[3].amount, 100)"
            >
              {{ numFloor(ovData[3].amount, 100) }}
            </h1>
          </div>
        </div>
        ...
        <div class="aveage-box">
          ...
          <div class="data-view-item">
            ...
            <div class="vertical-children mgt-10">
              ...
              <h1
                class="stroke3 dib mgl-5"
                :data-text="ovData[5].amount"
                style="color: #fed004"
              >
                {{ ovData[5].amount }}
              </h1>
        ...
        <div class="vertical-children mgt-5">
          ...
          <span class="mgl-10" style="color: #f3b893">{{
            $t("Festival_55")
          }}</span>
        </div>
        <section class="aveage-box mgt-10" id="my-res">
          <div class="vertical-children">
            ...
            <h1
              class="stroke3 dib mgl-10"
              :data-text="christmasData.balances.mbox"
            >
              {{ christmasData.balances.mbox }}
            </h1>
          </div>
          <div class="vertical-children">
            <img src="@/assets/christmas/mec.png" alt="" height="40" />
            <h1
              class="stroke3 dib mgl-5 mgl-10"
              :data-text="christmasData.balances.mec"
            >
              {{ christmasData.balances.mec }}
            </h1>
          </div>
          <div class="vertical-children">
            ...
            <h1
              class="stroke3 dib mgl-5 mgl-10"
              :data-text="christmasData.balances.box_v"
            >
              {{ christmasData.balances.box_v }}
            </h1>
          </div>
          <div class="tac" id="my-res-get-btn">
            <div class="img-btn" style="width: 130px" @click="claimMbox">
              ...
              <label style="margin-top: -3px">
                <span class="btn-stroke" :data-text="$t('Festival_17')">{{
                  $t("Festival_17")
                }}</span>
              </label>
            </div>
      ...
      <section class="mgt-20">
        <div class="por">
          <div class="dib por">
            <img
              class="show-record"
              src="@/assets/christmas/record.png"
              alt=""
              height="60"
              @click="
                getLucky();
            ...
            <img
              ref="pz"
              class="pz"
              src="@/assets/christmas/pz_a.png"
              alt=""
              width="550"
              v-if="Number(christmasData.balances.box_r) > 0"
            />
            <img
              ref="pz"
              class="pz"
              src="@/assets/christmas/pz.png"
              alt=""
              width="550"
              v-else
            />
        ...
        <h1
          class="stroke3"
          style="font-size: 30px; color: #ffedd1"
          :data-text="`${$t('Festival_52')}: ${christmasData.box}`"
        >
          {{ $t("Festival_52") }}: {{ christmasData.box }}
        </h1>
        <div class="tac mgt-10" id="center-opr">
          <div class="img-btn" style="width: 300px" @click="openBox(1)">
            ...
            <label style="margin-top: -3px" class="vertical-children">
              ...
              <span
                class="btn-stroke mgl-10"
                :data-text="$t('Festival_51').replace('#0#', 1)"
                style="font-size: 38px"
                >{{ $t("Festival_51").replace("#0#", 1) }}</span
              >
            </label>
          </div>
          <div class="img-btn mgl-20" style="width: 300px" @click="openBox(10)">
            ...
            <label style="margin-top: -3px" class="vertical-children">
              ...
              <span
                class="btn-stroke mgl-10"
                :data-text="$t('Festival_51').replace('#0#', 10)"
                style="font-size: 38px"
                >{{ $t("Festival_51").replace("#0#", 10) }}</span
              >
            </label>
          </div>
        </div>
      ...
      <img
        class="center-b"
        src="@/assets/christmas/center_b.png"
        alt=""
        width="100%"
      />
        ...
        <table id="christmas-table" class="tal">
          ...
          <tbody>
            <tr v-for="item in logs" :key="item.tx">
              ...
              <td v-if="item.types == 4">
                {{ $t("Festival_51").replace("#0#", item.num) }}
              </td>
              ...
              <td class="vertical-children hide-xs" v-if="item.types == 4">
                <div style="width: 30px; height: 30px" class="dib tac">
                  <img
                    :src="
                      require(`@/assets/christmas/${
                        reward[item.detail[0].gid].coin
                      }.png`)
                    "
                    alt=""
                    height="30"
                ...
                <span class="mgl-10"
                  >x{{ reward[item.detail[0].gid].amount }}</span
                >
              </td>
              <td v-if="item.types == 5" class="vertical-children hide-xs">
                <div style="width: 30px; height: 30px" class="dib tac">
                  <img
                    :src="
                      require(`@/assets/christmas/${item.detail[0].coin}.png`)
                    "
                    alt=""
                    height="30"
                  />
    ...
    <section class="result" v-if="isShowResult">
      <div class="result-body animate__bounceIn animate__animated">
        <img
          src="@/assets/christmas/close.png"
          class="result-close"
          alt=""
          height="50"
        ...
        <div class="result-cont" v-if="showReward.length == 1">
          ...
          <p
            class="stroke4 bold2"
            :data-text="`x ${reward[showReward[0].gid].amount}`"
            style="font-size: 50px; color: #fbecdf"
          >
            x {{ reward[showReward[0].gid].amount }}
          </p>
        </div>
        <div class="result-cont" v-else style="bottom: 30px">
          <div
            class="result-item ts"
            :class="{ active: reward[showReward[0].gid].needActive }"
          >
            <img
              :src="
                require(`@/assets/christmas/${
          ...
          <div
            class="result-item"
            v-for="(item, pos) in showReward.slice(1, 10)"
            :key="pos"
            :class="{ active: reward[item.gid].needActive }"
          >
            <img
              :src="require(`@/assets/christmas/${reward[item.gid].coin}.png`)"
              alt=""
            />
            <p
              class="stroke4 bold2"
              :data-text="`x ${reward[item.gid].amount}`"
              style="color: #fbecdf"
            >
              x {{ reward[item.gid].amount }}
            </p>
          </div>
        </div>
      </div>
    ...
    <section class="result" v-if="isShowRecord">
      <div
        class="result-body animate__zoomIn animate__faster animate__animated"
      >
        <img
          src="@/assets/christmas/close.png"
          class="result-close"
...
export default {
  ...
  methods: {
    ...
    showZp(list) {
      ...
      let angle =
        Common.getRandomNum(start - 18 + 3, end - 18 - 3) +
        (Math.ceil(this.preAngle / 360) + 1) * 360;

      $(this.$refs.pz).animate(
        { deg: angle },
        {
          duration: 5000,
          easing: "easeOutQuint",
    ...
    async getLucky() {
      let { data, status } = await axios.post(HttpConfig.Christmas.luckyGuys, {
        addr: this.myAddr,
      });
      if (status == 200) {
    ...
    async getLogs() {
      let { data, status } = await axios.post(HttpConfig.Christmas.Logs, {
        addr: this.myAddr,
      });
      if (status == 200) {
        this.logs = data.data;
    ...
    async getOvData() {
      let { data, status } = await axios.post(HttpConfig.Christmas.Ovdata, {
        addr: this.myAddr,
      });
      if (status == 200) {
    ...
    async claimMbox() {
      ...
      let { mbox, mec, box_v } = this.christmasData.balances;
      let canCliam =
        this.endTs < this.nowTs ||
        (this.endTs > this.nowTs &&
          (Number(mbox) >= 3 || Number(mec) >= 500 || Number(box_v) >= 1));
      if (!canCliam) {

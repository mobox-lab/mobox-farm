  ...
  <Dialog id="gem-apply2-dialog" :top="100" :width="400">
    ...
    <div class="ly-input-content por">
      <div class="tac">
        ...
        <div
          class="dib tac mgt-10"
          style="background: #000; border-radius: 20px; padding: 5px"
        >
          <div
            v-for="item in [101, 201, 301, 401]"
            :key="item"
            class="tac gem-select-item dib cur-point"
            :class="{ active: item == selectGemType }"
            @click="selectGemType = item"
          >
            <img
              :src="require(`@/assets/icon/${gemTypeToIcon[item]}.png`)"
              alt=""
              height="100%"
            />
          </div>
      ...
      <div class="por mgt-5">
        <div class="ly-input-pre-icon">
          <img
            :src="require(`@/assets/icon/${gemTypeToIcon[selectGemType]}.png`)"
            alt=""
          />
        </div>
        <input
...
export default {
  ...
  methods: {
    ...
    async submitApply(fcName) {
      ...
      let hash = await Wallet.ETH.Group.Gem.applyForGem(
        fcName,
        type,
        this.inputNum,
        () => {

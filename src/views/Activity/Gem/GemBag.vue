...
<template>
  <Dialog id="gemBag-dialog" :top="100" :width="450" :animation="isApprove">
    <ul class="gem-tab">
      <li
        v-for="item in gemType"
        :key="item"
        :class="['border_' + item, { active: item == selectGemType }]"
        class="tab-menu"
        @click="
          selectGemLv = 1;
          needCompNum = 1;
        ...
        >
          <div
            ...
            :class="[
              ...
              { 'opa-6': gemBag[Number(selectGemType) + item + item2] <= 0 },
            ]"
          ...
          >
            <img
              :src="
                require(`@/assets/market/${
                  Number(selectGemType) + item + item2
                }.png`)
              "
              alt=""
              width="100%"
            ...
            <span class="gem-num stroke bold">{{
              gemBag[Number(selectGemType) + item + item2]
            }}</span>
          </div>
        </div>
      </div>
        ...
        <div class="tar">
          <div class="gem-item dib" style="width: 70px; height: 70px">
            <img
              :src="
                require(`@/assets/market/${
                  Number(selectGemType) + selectGemLv
                }.png`)
              "
              alt=""
              width="100%"
            />
            <span class="gem-num stroke bold"
              >{{ getCompObj.consumeNum }}/3</span
            >
          </div>
        </div>
        <div style="flex: 2" class="tac">
          <div>
            <button class="btn-primary btn-small" @click="needCompNum = 1">
              Min</button
            >&nbsp;
            <SelectNum
              :needHideReduce="false"
              :maxNum="getCompObj.maxComp"
              :defaultNum="needCompNum"
              :data="{ needGemLv: selectGemLv - 1 }"
              :onChange="onNumChange"
            />&nbsp;
            <button
              class="btn-primary btn-small"
              @click="needCompNum = getCompObj.maxComp"
            >
              Max
            </button>
          </div>
          ...
          >
            ...
            <StatuButton
              data-step="2"
              class="mgt-10"
              :onClick="compGem.bind(this)"
              style="width: 70%"
              :isDisable="
                !hasApprove ||
                needCompNum <= 0 ||
                needCompNum > getCompObj.maxComp
              "
              :isLoading="lockBtn.compGemLock > 0"
            >
              {{ $t("Gemstone_25") }}
            </StatuButton>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      needCompNum: 1,
      hasApprove: -1,
      nowMaxLv: ConstantConfig.NOW_MAX_GEM_LEVEL, //
    };
  },
  watch: {
  ...
  methods: {
    ...
    onNumChange(data, num) {
      ...
      if (this.needCompNum >= this.getCompObj.maxComp)
        this.needCompNum = this.getCompObj.maxComp;
    },
  },
};
</script>

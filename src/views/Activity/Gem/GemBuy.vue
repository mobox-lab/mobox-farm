    ...
    <div class="por ovh mgt-20 adv-panel">
      <section class="col-md-7" style="padding: 20px">
        <div :class="{ veapply: tabPos == 1 }">
          ...
          <h1 class="vertical-children mgt-20">
            ...
            <img
              ...
              @click="oprDialog('gem-rule-dialog', 'block')"
          ...
          <div class="tac mgt-10">
            ...
            <template v-else>
              ...
              <template v-else>
                <p v-if="getCountDown > 0">
                  {{
                    Number(applyInfo.startTime) > nowTs
                      ? $t("Gemstone_53")
                      : $t("Gemstone_21")
                  }}: {{ getLeftTime(getCountDown) }}
                </p>
                <p v-else>
            ...
            <div class="aveage-box mgt-10" id="show-num">
              <div class="mgl-10 tac">
                {{ tabPos == 0 ? $t("Gemstone_62") : $t("Gemstone_64") }}:
                {{ getNowApplyCount[0] }}/{{ getNowApplyCount[1] }}
              </div>
            </div>
            <div class="aveage-box" style="margin-bottom: 20px">
              <div v-for="item in gemArr" :key="item.id" class="gem-apply-item">
                <div class="por">
                  <img
                    :src="require(`@/assets/market/${item.id}.png`)"
                    alt=""
                    width="100%"
                  />
                  <div
                    class="has-apply"
                    :class="{ active: myApplyGemType[tabPos] == item.id }"
                  >
                    <div>
                      <img
                        src="@/assets/icon/hasapply.png"
                        alt=""
                        height="30"
                      />
                      <p>{{ $t("Gemstone_67") }}</p>
                ...
                <span
                  class="apply-rate"
                  v-if="
                    Number(applyCfg[tabPos].now[item.name]) <
                    Number(applyCfg[tabPos].max)
                  "
                  ><span class="hide-xs">{{ $t("Gemstone_24") }}:</span
                  >100%</span
                >
        ...
        <div class="visiable-xs">
          <div class="mgt-30 aveage-box">
            <div class="dib por tac">
              <button
                ...
                @click="
                  setAction(22021);
                  oprDialog('gem-apply2-dialog', 'block');
                "
              >
                {{ $t("Gemstone_14") }}
              </button>
            ...
            <div class="tac">
              <button
                ...
                @click="
                  setAction(22001);
                  oprDialog('gem-take-dialog', 'block');
                "
              >
          ...
          <div
            class="gemBag"
            @click="oprDialog('gemBag-dialog', 'block')"
            style="bottom: 10px"
          >
        ...
        <div class="por" style="height: 380px; padding: 10px">
                ...
                <h3 class="por">
                  ...
                  >
                    <span
                      class="notice"
                      style="zoom: 0.7"
                      v-if="coinArr['GOV'].veMbox.notice && !hasStake"
                    ></span>
                    <img
              ...
              <div>
                <p class="small vertical-children">
                  ...
                  >
                    ...
                    />
                    <span
                      class="popMsg left tac"
                      style="min-width: 80px; text-align: center"
                      v-html="$t('Gemstone_67')"
                    ></span>
                  </span>
                </p>
          ...
          <div class="mgt-30 aveage-box">
            <div class="dib por tac">
              ...
              <p
                style="position: absolute; width: 200%; left: -50%"
                class="cur-point"
                @click="oprDialog('gem-rule-dialog', 'block')"
              >
                {{ $t("Gemstone_02") }}>>
        ...
        <table class="small new-table" border="0" frame="void" rules="none">
          ...
          <tr v-for="item in getHistory" :key="item.tx">
            ...
            <td class="vertical-children">
              <span v-if="item.isOver == false" style="color: #49c773">{{
                $t("Gemstone_30")
              }}</span>
    ...
    <Dialog id="gem-take-dialog" :top="100" :width="350">
                ...
                <img
                  :src="
                    require(`@/assets/market/${
                      gemTypeToNum[getTakeArr[item + item2].type] +
                      getTakeArr[item + item2].level
                    }.png`)
                  "
                  alt=""
                  width="100%"
      ...
      <div class="mgt-20">
        <StatuButton
          style="width: 60%"
          :onClick="takeGem"
          :isDisable="getTakeArr.length == 0"
          :isLoading="lockBtn.takeGemLock > 0"
        ...
        <div v-for="item in getRecordNoArr" :key="item.number">
          ...
          <div class="aveage-box tab-content small" style="padding: 8px">
            ...
            <p class="tar" v-if="historyDitail.isOver == true">
              <span v-if="item.isWins"
                ><img
                  :src="require(`@/assets/market/${historyDitail.gemId}.png`)"
                  alt=""
                  height="30"
              /></span>
  ...
  data() {
    return {
      ...
      applyCfg: {
        0: {
          max: 0,
          now: { red: 0, green: 0, blue: 0, gold: 0 },
  ...
  computed: {
    ...
    getRecordNoArr() {
      ...
      for (let index = 0; index < amountGem; index++) {
        ...
        retArr.push({
          number,
          isWins: wins[number] != undefined,
          type: this.gemType[item.roundIndex % 4],
        });
      }
      return retArr;
    },
  ...
  methods: {
    jumpVeMBOX() {
      if (this.hasStake) {
        ...
        this.$root.$children[0].$refs.vmbox
          .setOprData(this.coinArr["GOV"])
          .show();
      }
    },
    resizeGem() {
    ...
    async getApplyDetial(item) {
      ...
      let result = await Http.getGemApplyResult(
        this.account,
        roundIndex,
        applyType
      );
    ...
    async getApplyInfo() {
      ...
      if (result) {
        let {
          maxHashRateLuckyAmount,
          maxVeMoboxLuckyAmount,
          nowHashRateAmounts,
          nowVeMoboxAmounts,
        } = result;

    ...
    <div class="por clear mgt-10">
        ...
        <div class="adv-panel">
          <h1 class="vertical-children">
            ...
            <img
              ...
              @click="oprDialog('lottery-rule-dialog', 'block')"
          ...
          <div class="tac mgt-10">
            ...
            <template v-if="getNowBlock != '-'">
              <p
                class="small opa-6"
                v-if="
                  momoDatas[getNowRound - isShowPastRound].block >
                  Number(getNowBlock)
                "
              >
                {{ $t("Auction_04") }}:
                {{
                  getLeftTime(
                    momoDatas[getNowRound - isShowPastRound].ts - nowTs
                  )
                }}
              </p>
            ...
            <div class="mgt-10" v-if="getNowBlock != '-'">
              <PetItem
                v-bind:data="{ item: momoDatas[getNowRound - isShowPastRound] }"
              />
            </div>
      ...
      <section class="col-md-5" style="padding: 10px">
        <div class="panel por" style="height: 440px; padding: 20px">
          <div
            class="aveage-box tac"
            style="border-bottom: 1px solid #162340; padding-bottom: 10px"
          >
            <div class="por">
            ...
            <div>
              ...
              <div v-if="myLotteryData.ticket_number_start == '-'">
                <Loading />
              </div>
              <template v-else>
                <h4
                  class="notice-color"
                  v-if="
                    momoDatas[getNowRound - isShowPastRound].ts - 3600 - nowTs >
                    0
                  "
                >
                  -
                ...
                <template v-else>
                  <h4
                    class="notice-color"
                    v-if="myLotteryData.lottery_ticket < 1"
                  >
                    -
                  </h4>
                  <h4 v-else class="notice-color">
                    {{ myLotteryData.ticket_number_start }}-{{
                      myLotteryData.ticket_number_end
                    }}
                  </h4>
                </template>
              </template>
          ...
          <div class="tac mgt-5">
            ...
            <router-link to="/">
              <button class="btn-primary mgt-5" style="width: 50%">
                {{ $t("Auction_06") }}
              </button>
            </router-link>
          </div>
          ...
          <div class="mgt-10 tal">
            ...
            <table class="small new-table tac">
              ...
              <tr v-for="item in luckerArr" :key="item.issue_number">
                <td class="tal">
                  <PetItemMin :data="momoDatas[item.issue_number]" />
                </td>
                <td>
                  <p>
                    {{ dateFtt("yyyy-MM-dd", new Date(item.uptime * 1000)) }}
                  </p>
                ...
                <td>
                  <a
                    ...
                    :href="getBlockUrl(item.lottery_block_number)"
                    target="_blank"
                  >
      ...
      <div class="col-md-12 por" style="padding: 10px">
        <div class="tal mgt-10" v-if="getNowRound > 1">
          <div
            class="tab-menu"
            :class="{ active: isShowPastRound == 1 }"
            style="margin-left: 20px"
            @click="
              isShowPastRound = 1;
              getLotteryRank();
              getMyLotteryData(true);
            "
          >
            {{ $t("Auction_12") }}
          </div>
          <div
            class="tab-menu"
            :class="{ active: isShowPastRound == 0 }"
            @click="
              isShowPastRound = 0;
              getLotteryRank();
              getMyLotteryData(true);
          ...
          <div class="aveage-box">
            <div style="padding: 10px">
              ...
              <input
                ...
                :value="
                  momoDatas[getNowRound - isShowPastRound].ts - 3600 - nowTs > 0
                    ? getLeftTime(
                        momoDatas[getNowRound - isShowPastRound].ts -
                          3600 -
                          nowTs
                      )
                    : $t('Auction_15')
            ...
            <div style="padding: 10px">
              ...
              <input
                ...
                readonly
                class="ly-input mgt-10 tac"
                :value="momoDatas[getNowRound - isShowPastRound].block"
              />
            </div>
            <div style="padding: 10px">
              ...
              <p class="ly-input mgt-10 tac" style="line-height: 35px">
                <a
                  style="text-decoration: underline"
                  :href="
                    getBlockUrl(momoDatas[getNowRound - isShowPastRound].block)
                  "
                  target="_blank"
                  v-if="momoDatas[getNowRound - isShowPastRound].hash != '-'"
                >
                  {{
                    shortHash(momoDatas[getNowRound - isShowPastRound].hash)
                  }}</a
                >
                <span v-else>-</span>
              </p>
            ...
            <div style="padding: 10px">
              ...
              <input
                ...
                readonly
                class="ly-input mgt-10 tac"
                :value="rankListObj.total_lottery_ticket"
              />
            </div>
          </div>
        </section>
          ...
          <div style="min-height: 450px">
            <table class="small new-table">
              ...
              <tr v-for="item in rankListObj.list" :key="item.address">
                <td class="tal">
                  <span
                    class="dib tac rank-icon"
                    :class="{ active: item.rank <= 3 }"
                    >{{ item.rank }}</span
                  >
                  <span class="mgl-5">
                ...
                <td class="vertical-children tar tac-xs">
                  ...
                  >
                  <span v-else
                    >{{ item.ticket_number_start }}-{{
                      item.ticket_number_end
                    }}</span
                  >
                </td>
              </tr>

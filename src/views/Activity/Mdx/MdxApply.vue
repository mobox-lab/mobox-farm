<template>
	<Dialog id="mdx-apply-dialog" :top="100" :width="400">
		...
		<div class="ly-input-content por">
			<p class="small tal">{{$t("Air-drop_238") }}: {{$parent.getMaxApplyTimes}}</p>
			<div class="por mgt-5">
				...
				<input class="ly-input dib" type="text" style=" text-align: center; width: 70%; padding-left: 50px; "
					v-int :data-max="$parent.getMaxApplyTimes" data-min="1" v-model="inputNum" :placeholder="$t('NewBOX_05')" />
				<div class="dib" style="width: 30%">
					<button @click="inputNum = $parent.getMaxApplyTimes" class="btn-primary btn-small" style="width: 80%" >
						Max
					</button>
				</div>
			</div>
		...
		<div class="mgt-50">
			<span v-if="Number(applyInfo.startTime)  > nowTs ">{{$t("Gemstone_57")}}: {{getLeftTime(1631678400 - nowTs )}}</span>
			<template v-else>
				<p v-if="!isStartApply">{{$t("NewBOX_21")}}<span class="dotting"></span></p>
			</template>

			<div  >
				<StatuButton :isLoading="lockBtn.applyMdxBoxLock > 0" :isDisable="!isCanApply"  class="mgt-10" style="width:70%"  :onClick="applyForBox">
...
export default {
	...
	methods: {
		async applyForBox(){
			...
			let hash = await Wallet.ETH.Group.MdxBox.hApplyForBox(this.inputNum, ()=>{

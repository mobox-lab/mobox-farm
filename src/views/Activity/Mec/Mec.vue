        ...
        <div class="por">
          <div class="vertical-children">
            ...
            <img
              ...
              @click="getRootRefs().ruleDialog.show('MECBOX_02', 'MECBOX_03')"
          ...
          <div class="tac mgt-10">
            ...
            <div class="aveage-box mgt-10" id="show-num">
              <div class="tac mgl-10">
                {{ $t("MECBOX_37") }}: {{ getVeMboxApplyCount[0] }}/{{
                  Number(applyCfg[1].max)
                }}
              </div>
            </div>
            <div class="aveage-box mgt-5" id="show-num">
              <div class="tac mgl-10">
                {{ $t("MECBOX_35") }}: {{ getHashApplyCount[0] }}/{{
                  Number(applyCfg[0].max)
                }}
              </div>
            </div>
            ...
            <div class="mgt-10 aveage-box">
              <div class="dib por tac">
                <button
                  ...
                  @click="oprDialog('mecBox-apply-dialog', 'block')"
                >
                  {{ $t("MECBOX_14") }}
                </button>
              </div>
              <div class="tac">
                <StatuButton
                  v-if="this.myApplyInfo.boxAmount >= 0"
                  style="width: 80%"
                  :isDisable="this.myApplyInfo.boxAmount <= 0"
                  :isLoading="lockBtn.takeBoxLock > 0"
                  :onClick="takeBox"
                >
                  {{
            ...
            <div class="aveage-box tal" style="padding: 15px">
              <div>
                ...
                <h3>
                  <span
                    class="apply-rate"
                    v-if="Number(applyCfg[1].now) <= Number(applyCfg[1].max)"
                    >100%</span
                  >
                  <span class="apply-rate" v-else
                    >{{
                      numFloor(
                        (Number(applyCfg[1].max) / Number(applyCfg[1].now)) *
                          100,
                        1e2
                      )
                    }}%</span
              ...
              <div>
                ...
                <h3 class="por">
                  ...
                  <span class="apply-rate" v-else
                    >{{
                      numFloor(
                        (Number(applyCfg[0].max) / Number(applyCfg[0].now)) *
                          100,
                        1e2
                      )
                    }}%</span
      ...
      >
        <table class="small new-table" border="0" frame="void" rules="none">
          ...
          <tr v-for="item in getHistory" :key="item.tx">
            ...
            <td class="tal">
              <span v-if="Number(item.ticketStartNo) > 6e8">{{
                $t("Gemstone_52")
              }}</span>
    ...
    <Dialog id="mecBox-num-result-dialig" :top="100" :width="450">
      ...
      <div class="tab-body mgt-10">
        ...
        >
          ...
          <span v-if="historyDitail.item.amountBox > 1"
            >~{{
              Number(historyDitail.item.ticketStartNo) +
              Number(historyDitail.item.amountBox - 1)
            }}</span
          >
        </p>
        <p
          class="mgt-10 tal vertical-children"
          style="
            height: 40px;
            line-height: 38px;
            padding-left: 15px;
            color: #fff;
            font-size: 12px;
            background: rgba(161, 250, 64, 0.1);
            border: 2px solid #a1fa40;
            border-radius: 10px;
          "
        >
          <span>{{ $t("MECBOX_18") }}: </span>
          <span v-if="historyDitail.isOver == true">{{
            Object.keys(historyDitail.wins).length
          }}</span>
        ...
        <div style="padding: 10px" class="tal vertical-children">
          ...
          >
            <p v-if="historyDitail.isOver == true">
              <span v-if="Object.keys(historyDitail.wins).length == 0">{{
                $t("NewBOX_35")
              }}</span>
              <span
                v-else
                class="dib small"
                v-for="item in Object.keys(historyDitail.wins)"
                :key="item"
                style="
                  background: #2a2f35;
                  width: 30%;
    ...
    return {
      ...
      applyCfg: {
        0: {
          max: 0,
          now: 0,
          numCfg: { 0: 1600, 2e4: 3200, 4e4: 4800, 6e4: 6400, 12e4: 8000 },
        },
        1: {
          max: 0,
          now: 0,
          numCfg: { 0: 2400, 1e4: 4800, 2e4: 7200, 3e4: 9600, 6e4: 12000 },
        },
      },
      showRank: false,
    };

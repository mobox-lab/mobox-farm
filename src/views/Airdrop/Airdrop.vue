  ...
  <div id="aridorp" class="tac" style="padding-top: 20px">
    ...
    <section v-if="needMigrate || $route.query.old == 1">
      <div class="tac mgt-10" id="airdorp-top-menu">
        <div
          @click="$store.commit('bnbState/setData', { pledgeType: 'v2' })"
          :class="pledgeType == 'v2' ? 'active' : ''"
          class="tab-menu"
        >
          {{ $t("Air-drop_135") }}
        </div>
        <div
          @click="$store.commit('bnbState/setData', { pledgeType: 'v1' })"
          :class="pledgeType == 'v1' ? 'active' : ''"
          class="tab-menu por"
        >
          {{ $t("Air-drop_134") }}
          <span class="notice" v-if="needMigrate"></span>
          <button
            class="pop-notice animate__animated animate__headShake animate__infinite hide"
          ></button>
        </div>
      </div>
    </section>
    ...
    <div
      style="max-width: 1300px; margin: 0px auto; margin-bottom: 10px"
      class="mgt-20"
    >
      <p class="tal">
      ...
      <div class="panel">
        <div class="aveage-box tal" id="airdrop-cont-info">
          ...
          <div class="dib por tac" id="oneday-key" style="padding: 0px 10px">
            <h3 class="mgt-10 small vertical-children">
              <span class="opa-6">{{ $t("Air-drop_21") }}</span>
              <span
                class="mgl-5 cur-point dib"
                style="height: 25px"
                @click="oprDialog('mbox-drop-des-dialog', 'block')"
              >
                <svg
                  width="20"
                  height="25"
            ...
            <h3 class="mgt-5 vertical-children">
              <span>
                {{
                  pledgeType == "v1" && !(airdropCountDown > 0)
                    ? "-"
                    : totalAirdropKey
                }}
                MBOX
          ...
          <div class="dib tac" id="my-key" style="padding: 0px 10px">
            ...
            <div class="mgt-5">
              <button
                class="btn-success btn-small por"
                @click="$refs.keyopr.showAll()"
              >
                {{ getTotalKey }} MBOX
              </button>
            </div>
          ...
          <div class="tar">
            <a
              id="recheck"
              href="https://www.certik.org/projects/mobox"
              target="_blank"
            >
    ...
    <section id="airdrop-cont" class="mgt-20 por">
      ...
      >
        <div
          class="ly-checkbox"
          @click="
            $store.commit('bnbState/setData', {
              onlyShowPledge: !onlyShowPledge,
            })
          "
          :class="onlyShowPledge ? 'active' : ''"
        >
      ...
      <div class="hide">
        ...
        <p class="vertical-children">
          ...
          <span
            class="mgl-5 cur-point dib"
            style="height: 20px"
            @click="oprDialog('veMbox-des-dialog', 'block')"
          >
            <svg
              width="20"
              height="20"
      ...
      <div class="row mgt-20" style="padding: 4px">
        <div
          class="col-md-4 col-xs-12 col-sm-6 mgt-10"
          style="margin-bottom: 10px"
          v-if="needShowRefund"
        >
          <div class="airdrop-item por">
            <section style="padding: 20px">
              <h3>{{ $t("Air-drop_217") }}</h3>
              <div class="aveage-box mgt-20">
                <p class="tal opa-6">{{ $t("Air-drop_173") }}</p>
                <p class="tar notice-color">
                  {{ numFloor(getNeedTransferVeMbox[0] / 1e18, 1e2) }} veMBOX
                </p>
              </div>
              <div class="aveage-box mgt-10">
                <p class="tal opa-6">{{ $t("Air-drop_174") }}</p>
                <p class="tar notice-color">
                  {{ numFloor(getNeedTransferVeMbox[1] / 1e18, 1e2) }} veMBOX
                </p>
              </div>
              <div class="aveage-box mgt-10">
                <p class="tal opa-6">{{ $t("Air-drop_175") }}</p>
                <p class="tar notice-color">
                  {{ numFloor(getNeedTransferVeMbox[2] / 1e18, 1e2) }} veMBOX
                </p>
              </div>
              <p class="small mgt-10 color-buy tal">{{ $t("Air-drop_218") }}</p>
              <button
                class="btn-primary mgt-10"
                @click="
                  $parent.$refs.vmbox.setOprData(getHasVeMboxPool).show(1)
                "
              >
                {{ $t("Air-drop_154") }}
              </button>
            ...
            <section style="padding: 20px">
              <h3>{{ $t("Air-drop_219") }}</h3>
              <div class="aveage-box mgt-20">
                <p class="tal opa-6">{{ $t("Air-drop_173") }}</p>
                <p class="tar notice-color">{{ refundData.short }} MBOX</p>
              </div>
              <div class="aveage-box mgt-10">
                <p class="tal opa-6">{{ $t("Air-drop_174") }}</p>
                <p class="tar notice-color">{{ refundData.middle }} MBOX</p>
              </div>
              <div class="aveage-box mgt-10">
                <p class="tal opa-6">{{ $t("Air-drop_175") }}</p>
                <p class="tar notice-color">{{ refundData.long }} MBOX</p>
              </div>
              <p class="mgt-10">
                {{ $t("Air-drop_220") }}:
                <span class="notice-color">{{ refundData.total }} MBOX</span>
              </p>
              <StatuButton
                v-if="refundData.refunded == false"
                :isLoading="lockBtn.refundMboxLock > 0"
                :onClick="refundMbox"
                class="mgt-10"
                >{{ $t("Air-drop_221") }}</StatuButton
              >
              <StatuButton v-else :isDisable="true" class="mgt-10">{{
                $t("Air-drop_222")
              }}</StatuButton>
            </section>
          </div>
        ...
        <div
          class="col-md-4 col-xs-12 col-sm-6 mgt-10"
          style="margin-bottom: 10px"
          v-for="item in getPledgeList"
          :key="item.coinName + item.addr"
        >
          <div
            class="airdrop-item tal"
            :class="
              `pledgeType-${pledgeType} ` +
              (item.coinName == 'MBOX-BNB' ? 'active' : '')
            "
          >
            <div class="test"></div>
            <div style="padding: 20px" class="por">
              ...
              <div class="over" v-if="item.coinKey != 'MBOX-BNB-V2'">
                {{ $t("Air-drop_134") }}
              </div>

              <div
                class="close point-block"
                @click="toggleClass($event, { isLP: true })"
              >
                <div class="vertical-children" style="padding-left: 56px">
                  <div
                    class="dib airdorp-item-coin-icon tac"
                    :class="item.isLP ? 'double-img' : ''"
                  >
                    <img
                      v-for="(name, key) in item.coinName.split('-')"
                      :key="name + key"
                  ...
                  <div style="margin-left: 15px">
                    <h3 class="color-w tal">
                      {{ item.coinName }} {{ item.isLP ? "LP" : "POOL" }}
                      <sub v-if="item.isLP"
                        >({{ item.pancakeVType == 1 ? "V1" : "V2" }})</sub
                      >
                    </h3>
                ...
                <div
                  class="mgt-10 small toggle-item"
                  style="background: #13181f; padding: 5px; border-radius: 10px"
                >
                  <div
                    class="aveage-box vertical-children por"
                    style="padding: 2px"
                  >
                  ...
                  >
                    ...
                    <p class="dib tar">
                      {{
                        numFloor(
                          (item.veMoboxSupply / item.veMboxTotal) * 100,
                          100
                        )
                      }}%
                    </p>
              ...
              <template v-if="pledgeType == 'v1'">
                <p
                  class="tac small mgt-10"
                  style="color: #49c773"
                  v-if="airdropCountDown > 0 && item.coinKey == 'MBOX-BNB'"
                >
                  ({{
                    $t("Air-drop_149").replace(
                      "#0#",
                      getLeftTime(airdropCountDown)
                    )
                  }})
                </p>
              </template>
              <template v-else>
                <p
                  class="tac small mgt-10"
                  style="color: #49c773"
                  v-if="airdropCountDown > 0 && item.coinKey == 'MBOX-BNB-V2'"
                >
                  ({{
                    $t("Air-drop_150").replace(
                      "#0#",
                      getLeftTime(airdropCountDown)
                    )
                  }})
              ...
              <div v-if="item.coinKey != 'MBOX-BNB-V2'" class="mgt-50 close">
                <div class="aveage-box">
                  <div>
                    <span class="opa-6">{{ $t("Air-drop_205") }}</span>
                  </div>
                  <div class="tar" style="flex: 2">
                    <span
                      class="vertical-children"
                      :class="item.isLP ? 'show-point-block' : ''"
                    >
                      <span>-</span>
                    </span>
                  </div>
              ...
              <div
                v-else
                class="point-block mgt-50 close"
                @click="
                  getApyObj(item);
                  toggleClass($event, { isLP: true });
                "
              >
                <div class="aveage-box">
                  <div>
                ...
                >
                  ...
                  >
                    <p class="dib">CAKE</p>
                    <p class="dib tar">
                      <span v-if="item.myApy.cake != '-'"
                        >{{ numFloor(item.myApy.cake * 100, 100) }}%</span
                      >
                      <Loading v-else />
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <p class="dib tar">
                      <span v-if="item.myApy.lpFee != '-'"
                        >{{ numFloor(item.myApy.lpFee * 100, 100) }}%</span
                      >
                      <Loading v-else />
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <p class="dib tar">
                      <span v-if="item.myApy.key != '-'">
                        {{
                          numFloor(
                            (Number(item.myApy.key) +
                              Number(item.myApy.cake) +
                              Number(item.myApy.lpFee)) *
                              100,
                            100
                          )
                        }}%
                  ...
                  <div
                    v-if="item.wantAmount > 0 && item.maxApyNeedVeMobox != '-'"
                    class="tac mgt-10"
                  >
                    <p v-if="item.veMbox.mul < 300">
                      <span
                        v-html="
                          $t('Air-drop_203').replace(
                            '#0#',
                            `<span class='color-sell'>${numFloor(
                              item.maxApyNeedVeMobox,
                              1e2
                            )}</span>`
                          )
                        "
                      ...
                      <span v-if="item.wantAmount > 0" class="color-sell">
                        {{
                          numFloor(
                            (Number(item.maxKeyApy) + Number(item.myApy.cake)) *
                              100,
                            100
                          )
                        }}%
                      ...
                      <span v-else>
                        {{
                          numFloor(
                            (Number(item.allKeyApy) + Number(item.myApy.cake)) *
                              100,
                            100
                          )
                        }}%
              ...
              <div class="tal mgt-20">
                <p class="por mgt-10">
                  ...
                  <span class="suffix"
                    >${{ numFloor(item.totalSupply, 1).toLocaleString() }}</span
                  >
                </p>
              </div>
              ...
              <div class="tal mgt-20">
                <p class="por mgt-10">
                  ...
                  <span class="suffix">{{
                    numFloor(coinArr[item.coinKey].balance, 1e6) || "0"
                  }}</span>
                </p>
              </div>

              <div
                class="point-block mgt-20 close"
                @click="
                  getLPPrice(item);
                  toggleClass($event, item);
                "
              >
                <div class="aveage-box">
                  <div>
                  ...
                  <div class="tar" style="flex: 2">
                    <span
                      v-if="item.wantAmount > 0"
                      class="vertical-children notice-color"
                      :class="item.isLP ? 'show-point-block' : ''"
                    >
                    ...
                    <span v-else style="padding: 5px; display: inline-block"
                      >-</span
                    >
                  </div>
                </div>
                ...
                >
                  ...
                  >
                    ...
                    <p class="dib tar">
                      <span class="mgl-10" v-if="item.lpPrice[key] != '-'">{{
                        item.lpPrice[key]
                      }}</span>
                      <Loading v-else />
                      <img
                ...
                <p class="por mgt-10">
                  ...
                  <span class="suffix">
                    {{
                      numFloor(
                        (Number(
                          coinArr[item.coinKey].veMbox.orderIndexs[0].veMboxNum
                        ) +
                          Number(
                            coinArr[item.coinKey].veMbox.orderIndexs[1]
                              .veMboxNum
                          ) +
                          Number(
                            coinArr[item.coinKey].veMbox.orderIndexs[2]
                              .veMboxNum
                          )) /
                          1e18,
                        1e4
                    ...
                    <span
                      class="notice-color"
                      v-if="coinArr[item.coinKey].veMbox.mul > 100"
                      >({{
                        numFloor(coinArr[item.coinKey].veMbox.mul / 100, 100)
                      }}x)</span
                    >
                  </span>
                </p>
                ...
                <p class="por mgt-10">
                  ...
                  <span class="suffix">
                    <button
                      class="btn-success btn-small por"
                      :class="
                        lockBtn.getKeyLock > 0 || item.earnedKey == 0
                          ? 'disable-btn'
                          : ''
                      "
                      @click="$refs.keyopr.setCheckCoin(item.coinKey).show()"
                    >
                      <Loading
                        class="btn-loading"
                        v-if="lockBtn.getKeyLock > 0"
                      />
                      {{ item.earnedKey }} MBOX
                    </button>
                  </span>
              ...
              <div class="tac mgt-30">
                <template v-if="pledgeType == 'v2'">
                  <div
                    ...
                    @click="
                      setAction(21003);
                      needSetItem = item;
                      oprDialog('deposit-notice-dialog', 'block');
                    "
                    v-if="needShowNotice"
                  >
                    <img
                      class="cur-point"
                      width="40"
                      src="../../assets/icon/deposit_icon.png"
                      alt=""
                    />
                    <span
                      style="
                  ...
                  <div
                    ...
                    @click="
                      setAction(21003);
                      $refs.deposit.setOprData(item).show();
                    "
                    v-else
                  >
                    <img
                  ...
                  <div
                    ...
                    @click="
                      setAction(21004);
                      $refs.withdraw.setOprData(item).show();
                    "
                  >
                    <img
                      class="cur-point"
                      width="40"
                      src="../../assets/icon/withdraw_icon.png"
                      alt=""
                    />
                    <span
                      style="
                  ...
                  <div
                    class="dib por cur-point"
                    style="margin-left: 50px; width: 50px"
                    @click="
                      setAction(21005);
                      $parent.$refs.vmbox.setOprData(item).show();
                    "
                  >
                    <span
                      class="notice"
                      v-if="coinArr[item.coinKey].veMbox.notice"
                    ></span>
                    <img
                      src="@/assets/icon/vembox-icon.png"
                      alt=""
                      height="40"
                    />
                    <span
                ...
                <template v-else>
                  <button
                    ...
                    style="width: 60%"
                    @click="
                      migrateCoinKey = item.coinKey;
                      oprDialog('migrate-dialog', 'block');
                    "
                  >
                    {{ $t("Air-drop_136") }}
                  </button>
            ...
            <template v-if="item.isLP && pledgeType == 'v2'">
              ...
              <div style="padding: 20px">
                <div class="tac">
                  <button
                    :class="
                      item.coinName == 'MBOX-BNB' ? 'btn-primary' : 'btn-line'
                    "
                    style="width: 40%"
                    @click="
                      setAction(21001);
                      $root.$children[0].$refs.pancake
                        .setOprData(item)
                        .show('swap');
                    "
                  >
      ...
      <div class="aveage-box" style="background: #1c222c; border-radius: 10px">
        <div style="padding: 10px">
          ...
          <input
            type="text"
            readonly
            class="ly-input mgt-10 tac"
            :value="
              numFloor(totalStakeMbox, 1).toLocaleString() +
              '(' +
              numFloor((totalStakeMbox / buyBack.circulating) * 100, 100) +
              '%)'
            "
          />
        </div>
        <div style="padding: 10px">
          ...
          <input
            ...
            readonly
            class="ly-input mgt-10 tac"
            :value="numFloor(veMboxTotal / 1e18, 1)"
          />
        </div>
        <div style="padding: 10px">
        ...
        <div style="padding: 10px" class="por">
          ...
          <input
            ...
            readonly
            class="ly-input mgt-10 tac"
            :value="getMyTotalVeMbox"
          />
          <span
            class="dib por cur-point"
            style="width: 30px; position: absolute; right: 20px; bottom: 10px"
            @click="
              $root.$children[0].$refs.vmbox.setOprData(coinArr['GOV']).show()
            "
          >
            <span
              class="notice"
              style="zoom: 0.7"
              v-if="coinArr['GOV'].veMbox.notice"
            ></span>
    ...
    <section id="buy-back" class="mgt-20 por">
      ...
      <div class="aveage-box" style="background: #1c222c; border-radius: 10px">
        <div style="padding: 10px">
          ...
          <input
            ...
            readonly
            class="ly-input mgt-10 tac"
            :value="'$' + buyBack.amount.toLocaleString()"
          />
        </div>
        <div style="padding: 10px">
        ...
        <div style="padding: 10px">
          ...
          <input
            ...
            readonly
            class="ly-input mgt-10 tac"
            :value="buyBack.moboxBurn.toLocaleString()"
          />
        </div>
        <div style="padding: 10px">
          ...
          <input
            ...
            readonly
            class="ly-input mgt-10 tac"
            :value="buyBack.circulating.toLocaleString()"
          />
        </div>
      </div>
      <table
        class="table-his tac small mgt-20"
        style="width: 100%"
        border="0"
        frame="void"
        rules="none"
      >
        <tr>
          ...
          <th style="width: 25%" class="no-dropdown-bg">
            <Dropdown
              ...
              :list="burnTypeList"
              :defaultSelectPos="buyBackType"
              :onChange="onBurnTypeChange"
            />
          </th>
          <th>{{ $t("Air-drop_51") }}</th>
          <th>{{ $t("Air-drop_50") }}</th>
        ...
        <tr v-for="item in buyBack.logs" :key="item.txId">
          <td>
            {{ dateFtt("yyyy-MM-dd hh:mm:ss", new Date(item.ts * 1000)) }}
          </td>
        ...
        <tr>
          <td colspan="6">
            <a
              href="https://bscscan.com/address/0x9907fa65f3b0a9b9254a2c29a213c4d3501bf84f"
              target="_blank"
              >{{ $t("Air-drop_148") }}</a
            >
          </td>
    ...
    <Dialog id="migrate-dialog" :top="100" :width="500">
      <template v-if="migrateCoinKey != ''">
        <h3>
          {{ $t("Air-drop_137").replace("#0#", getMigrateItem.coinName) }}
        </h3>
        <div class="tab-panel mgt-20">
          <h3 class="tal">1, {{ $t("Air-drop_08") }}</h3>
          <div class="mgt-20">
            <p class="small">
              {{ $t("Air-drop_138") }}: {{ getMigrateItem.wantAmount }}
              {{ getMigrateItem.coinName }} LP V1
            </p>
            <StatuButton
              class="mgt-10"
              style="min-width: 150px"
              :isDisable="getMigrateItem.wantAmount <= 0"
              :onClick="() => $refs.withdraw.setOprData(getMigrateItem).show()"
            >
              {{ $t("Air-drop_08") }}
            </StatuButton>
          </div>
        </div>
        <svg viewBox="0 0 24 24" width="18px" class="mgt-10">
          <path
            fill="#838689"
        ...
        <div class="tab-panel mgt-10">
          ...
          <div class="mgt-20">
            <p class="small">
              {{ $t("Air-drop_139") }}:
              {{ numFloor(getMigrateItem.balance, 1e6) || "0" }}
              {{ getMigrateItem.coinName }} LP V1
            </p>
            <StatuButton
              class="mgt-10"
              :isDisable="getMigrateItem.balance <= 0"
              :onClick="
                () =>
                  $root.$children[0].$refs.pancake
                    .setOprData(getMigrateItem)
                    .show('liquidity')
        ...
        <div class="tab-panel mgt-10">
          ...
          <div class="mgt-20">
            <p class="small">
              {{ $t("Air-drop_112") }}: {{ getMigrateTargetItem.balance }}
              {{ getMigrateItem.coinName }} LP V2
            </p>
            <StatuButton
              class="mgt-10"
              style="min-width: 150px"
              :isDisable="getMigrateTargetItem.balance <= 0"
              :onClick="
                () => $refs.deposit.setOprData(getMigrateTargetItem).show()
              "
            >
              {{ $t("Air-drop_07") }}
            </StatuButton>
    ...
    <Dialog id="deposit-notice-dialog" :top="100" :width="400">
      <h2>{{ $t("Air-drop_23") }}</h2>
      <div
        class="tab-body tal mgt-10 small"
        v-html="$t('Air-drop_24')"
        style="padding: 15px"
      ></div>
      <div class="tab-body tal mgt-10" style="padding: 15px">
        <div class="vertical-children" style="display: flex">
          <div
            class="ly-checkbox"
            :class="hasAgreeNotice ? 'active' : ''"
            @click="hasAgreeNotice = !hasAgreeNotice"
          >
          ...
          &nbsp;
          <span class="small mgl-10" style="flex: auto">{{
            $t("Air-drop_25")
          }}</span>
        </div>
        <div
          class="vertical-children mgt-10"
          style="display: flex; justify-content: flex-start"
        >
          <div
            class="ly-checkbox"
            :class="hasSelectNotShow ? 'active' : ''"
            @click="hasSelectNotShow = !hasSelectNotShow"
          >
            <div style="width: 20px">
              <svg class="hide" viewBox="0 0 1024 1024" width="20" height="20">
                <path
          ...
          &nbsp;
          <span class="small mgl-10" style="flex: 1 auto">
            {{ $t("Air-drop_26") }}
          </span>
        </div>
        ...
        <div class="mgt-20 aveage-box">
          <div class="tac">
            <button
              class="btn-primary"
              style="width: 80%"
              @click="oprDialog('deposit-notice-dialog', 'none')"
            >
              {{ $t("Air-drop_27") }}
            </button>
          </div>
          <div class="tac">
            <button
              class="btn-primary"
              style="width: 80%"
              :class="hasAgreeNotice ? '' : 'disable-btn'"
              @click="agreeNotice"
            >
              {{ $t("Air-drop_28") }}
            </button>
    ...
    <Dialog id="veMbox-des-dialog" :top="100" :width="400">
      ...
      <div class="ly-input-content mgt-20">
        ...
        <div
          class="aveage-box"
          style="border-bottom: 1px solid #333; padding: 5px"
          v-for="(item, pos) in veMboxConfig"
          :key="pos"
        >
          <p class="tac">≤ {{ item.max * 100 }}%</p>
          <p class="tac">x{{ item.mul }}</p>
        </div>
      </div>
    ...
    <Dialog id="mbox-drop-des-dialog" :top="100" :width="420">
      ...
      <div class="ly-input-content mgt-20">
        ...
        <div
          class="aveage-box"
          style="border-bottom: 1px solid #333; padding: 5px"
          v-for="(item, pos) in mboxDropConfig"
          :key="pos"
        >
          <p class="tac">
  ...
  computed: {
    ...
    getPledgeList() {
      ...
      for (let key in stakeLP) {
        ...
        if (
          pIndex != -1 &&
          (this.coinArr[key].wantAmount > 0 || !this.onlyShowPledge)
        ) {
    ...
    getMigrateTargetItem() {
      ...
      retItem = {
        coinKey: targetCoinKey,
        ...PancakeConfig.StakeLP[targetCoinKey],
        ...this.coinArr[targetCoinKey],
      };
      return retItem;
    },
    getMigrateItem() {
    ...
    getHasVeMboxPool() {
      ...
      this.getPledgeList.map((item) => {
        if (item.coinName != "MBOX-BNB") {
          if (
            (item.veMbox.orderIndexs[0] != 0 ||
              item.veMbox.orderIndexs[0] != 0 ||
              item.veMbox.orderIndexs[2] != 0) &&
            retItem == null
          ) {
            retItem = item;
          }
    ...
    needShowRefund() {
      let totalVeMbox =
        this.numFloor(this.getNeedTransferVeMbox[0] / 1e18, 1e2) +
        this.numFloor(this.getNeedTransferVeMbox[1] / 1e18, 1e2) +
        this.numFloor(this.getNeedTransferVeMbox[2] / 1e18, 1e2);
      return (
        (Number(this.numFloor(totalVeMbox, 1e2)) > 0 &&
          Number(this.refundData.total) > 0) ||
        (this.refundData.refunded == false && Number(this.refundData.total) > 0)
      );
    },
  },
  async created() {
    let hasSelectNotShowNotice = Common.getStorageItem(
      "hasSelectNotShowNotice"
    );

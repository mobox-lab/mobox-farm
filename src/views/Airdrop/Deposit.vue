  ...
  <Dialog id="deposit-dialog" :top="100" :width="400">
      ...
      <div class="tab-content">
        ...
        <div class="mgt-10 por">
          ...
          <input
            ...
            class="ly-input tac"
            style="width: 100%"
            v-model="inputDepositValue"
            v-number
            :placeholder="$t('Air-drop_96')"
          />
        </div>
        <div class="aveage-box mgt-10">
          ...
          <div class="tar small" style="flex: 2">
            <p>
              {{ oprData.balance }} {{ oprData.coinName }}
              <span v-if="oprData.isLP"
                >(Pancake LP V{{ oprData.pancakeVType }})</span
              >
            </p>
          </div>
        </div>
        ...
        <div
          class="mgt-30 tac"
          :class="depositNeedApprove ? ' btn-group' : ''"
          style="margin-bottom: 10px"
          v-if="oprData.coinName == 'MBOX-BNB'"
        >
          <div v-if="depositNeedApprove">
            <StatuButton
              data-step="1"
              style="width: 70%"
              :isLoading="coinArr[oprData.coinKey].isApproving"
              :onClick="() => approveToPool(oprData.coinKey)"
            >
          ...
          <button
            ...
            style="width: 70%"
            :class="!isCanDeposit ? 'disable-btn' : ''"
            @click="deposit"
          >
            <Loading
              v-if="
                oprData.coinName != '' && coinArr[oprData.coinKey].isDeposing
              "
    ...
    <div class="tab-body tal mgt-10">
      <div class="tab-content">
        ...
        <div
          class="aveage-box opa-6 mgt-10 small"
          style="border-bottom: 1px solid #202c4a; height: 30px"
        >
...
export default {
  ...
  watch: {
    ...
    inputPercent: function (newData) {
      ...
      let targetValue = Common.numFloor(
        Number(balance) * Number(newData),
        omit
      );
      //
      if (newData == 1) {
  ...
  computed: {
    ...
    isCanDeposit() {
      ...
      return (
        Number(this.inputDepositValue) > 0 &&
        Number(this.inputDepositValue) <=
          this.getTrueShowBalacne(Number(oprCoin.balance)) &&
        !oprCoin.isDeposing &&
    ...
    depositNeedApprove() {
      ...
      return (
        coinName != "BNB" &&
        allowanceToPool >= 0 &&
        allowanceToPool / 1e18 < Number(this.inputDepositValue)
      );
    },
  },
  ...
  methods: {
    ...
    async setCoinAllowanceToPool(coinKey) {
      if (
        coinKey != "" &&
        coinKey != "BNB" &&
        this.coinArr[coinKey].allowanceToPool == -1
      ) {
        let allowance = await Wallet.ETH.viewErcAllowanceToTarget(
          PancakeConfig.StakeLP[coinKey].addr,

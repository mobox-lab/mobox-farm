		...
		<div class="tab-content">
			...
			<div class="mgt-20 tac " :class="needApproved?'btn-group':'' " style="margin-bottom:10px">
				<div v-if="needApproved && from.coinName != '' ">
					<button data-step="1" @click="approve" class="btn-primary por" style="width:80%;" :class="coinArr[from.coinName].allowanceToQuickSwap > 1e8 || coinArr[from.coinName].isApproving?'disable-btn':''">
...
export default {
	...
	computed:{
		...
		needApproved(){
			...
			if(coinKey != "" && coinKey != "BNB" && this.coinArr[coinKey].allowanceToQuickSwap == 0){
				needApprove = true;
			}
			return needApprove;
		},
	...
	methods:{
		...
		async setCoinAllowance(){
			...
			if(coinKey != "" && coinKey != "BNB") {
				let allowance = await Wallet.ETH.viewErcAllowanceToTarget(PancakeConfig.SelectCoin[coinKey].addr, contractAddr, false);
		...
		async approve(){
			...
			let contractAddr = PancakeConfig.QuickSwapAddr;
			let hash = await Wallet.ETH.approveErcToTarget(PancakeConfig.SelectCoin[coinKey].addr, contractAddr, {coinKey, type: "allowanceToQuickSwap"});
			if(hash){

  ...
  <ul class="list">
    <li v-for="(item, index) in list" :key="index">
      <div class="label">
        ...
        <div class="flex-1">
          <div class="material-science">{{$t('MOMOMerge_06')}}</div>
        </div>
      </div>
      <div class="momos selected">
        ...
        <div class="flex-1">
          <div class="item" v-for="(momo, index) in item.materialScience" :key="index">
            <Item :data="momo" />
          </div>
        </div>
      </div>
      ...
      <div class="momos result">
        ...
        <div class="info">
          <a class="tx" target="_blank" :href="'https://bscscan.com/tx/' + item.tx">TX</a>
    ...
    async getList() {
      ...
      if (data.list) {
        this.list = data.list.map((item) => {
          ...
          return {
            time: Common.dateFtt('yyyy-MM-dd hh:mm:ss', new Date(item.crtime * 1000)),
            tx: item.tx,
...
.tx {
  border-radius: 5rem;
  font-weight: bold;
  background: linear-gradient(90deg, rgba(61, 109, 251, 0), rgba(61, 109, 251, 0.1));
}

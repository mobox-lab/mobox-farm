<template>
  <div id="upgrade" style="margin-top: 10px">
          ...
          <div>
            ...
            <div class="tac">
              <div id="price" class="vertical-children">
                <img
                  :src="require(`@/assets/coin/${oprCoin}.png`)"
                  height="25"
                  alt=""
                />&nbsp;
                <span class="money">{{
                ...
                <div ::class="{'btn-group': needApprove}">
                  ...
                  <div v-if="needApprove">
                    <StatuButton
                      data-step="1"
                      style="width: 200px"
                      :isLoading="coinArr[oprCoin].isApproving"
                      :onClick="approve"
                      >{{ $t("Air-drop_16") }} {{ oprCoin }}</StatuButton
                    >
                  </div>
                  <div class="mgt-10">
                    ...
                    <StatuButton
                      data-step="2"
                      style="width: 200px"
                      :isDisable="needApprove || lockBtn.buyMomoLock > 0"
                      :onClick="
                        () => oprDialog('confirm-gem-buy-dialog', 'block')
                      "
                    >
                      {{ $t("Market_22") }}
                    </StatuButton>
              ...
              <div v-if="isMyPet" class="mgt-20">
                <StatuButton
                  v-if="this.getNowPetItem.currency == 2"
                  :onClick="setChangePriceData.bind(this, true)"
    ...
    <Dialog id="changePrice-dialog" :top="200" :width="400">
      ...
      <div class="mgt-20">
        <div class="ly-input-content mgt-10" v-if="!isGem">
          ...
          <div class="por mgt-5">
            ...
            <input
              v-model="sellObj.onePrice"
              class="ly-input sell-input"
              type="text"
              :placeholder="$t('Market_39')"
              v-number
              data-max="100000000"
            />
          </div>
        ...
        <div class="ly-input-content mgt-10">
          ...
          <div class="por mgt-5">
            ...
            <input
              ...
              v-model="totalPrice"
              readonly
              class="ly-input sell-input opa-6"
              type="text"
              :placeholder="$t('Market_18')"
              v-number
              data-max="100000000"
      ...
      <p class="color-buy tac mgt-10 small" v-if="Number(totalPrice) < 10">
        {{ $t("Market_77") }}
      </p>
      <StatuButton
        style="width: 200px"
        class="mgt-10"
        :onClick="changePrice"
        :isDisable="Number(totalPrice) < 10"
      >
        {{ $t("Common_03") }}
      </StatuButton>
    </Dialog>
    ...
    <Dialog id="confirm-gem-buy-dialog" :top="200" :width="350">
      <h4
        ...
        v-html="
          $t('Market_59').replace(
            '#0#',
            `<span style='color: #49c773' class='money'>${numFloor(
              sellObj.startPrice,
              1e2
            ).toLocaleString()} ${oprCoin}</span>`
          )
        "
      ></h4>
      <div class="mgt-50">
        <button
          class="btn-primary"
          @click="oprDialog('confirm-gem-buy-dialog', 'none')"
        >
          {{ $t("Common_04") }}
        </button>
        <button
          ...
          @click="
            oprDialog('confirm-gem-buy-dialog', 'none');
            buyPet();
          "
        >
          {{ $t("Common_03") }}
...
export default {
  ...
  computed: {
    ...mapState({
      tempGemMarketCancelTx: (state) =>
        state.marketState.data.tempGemMarketCancelTx,
    ...
    needApprove() {
      return (
        this.coinArr[this.oprCoin].allowanceToGemAuction / 1e18 <
          Number(this.getNowPetItem.price / 1e9) &&
        this.coinArr[this.oprCoin].allowanceToGemAuction != -1
      );
    },
  },
  async created() {
  ...
  methods: {
    ...
    async viewAllowance() {
      ...
      let allowanceToGemAuction = await Wallet.ETH.viewErcAllowanceToTarget(
        PancakeConfig.SelectCoin[coinKey].addr,
        WalletConfig.ETH.common1155Auction,
        false
      );
      if (allowanceToGemAuction) {
        this.coinArr[coinKey].allowanceToGemAuction = Number(
          allowanceToGemAuction
        );
    ...
    async approve() {
      ...
      let hash = await Wallet.ETH.approveErcToTarget(
        PancakeConfig.SelectCoin[coinKey].addr,
        WalletConfig.ETH.common1155Auction,
        { coinKey, type: "allowanceToGemAuction" }
      );
      if (hash) {
    ...
    async buyPet() {
      ...
      let coinKey = this.oprCoin;
      if (
        this.coinArr[coinKey].allowanceToGemAuction <= 0 ||
    ...
    async cancelAuction() {
      ...
      if (hash) {
        //
        this.tempGemMarketCancelTx.push({
          tx: this.getNowPetItem.tx,
          nextHash: hash,
          currency: this.getNowPetItem.currency,
        });

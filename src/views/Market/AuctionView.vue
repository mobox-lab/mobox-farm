  ...
  <div id="upgrade" style="margin-top: 10px">
          ...
          <div>
            ...
            <div class="tac">
              <div id="price" class="vertical-children">
                ...
                <span class="money">{{
                  numFloor(this.nowPrice / 1e9, 1e2).toLocaleString()
                }}</span>
              </div>
              <div
                v-if="
                  this.getNowPetItem.startPrice != this.getNowPetItem.endPrice
                "
              >
                <div class="jdt">
                  ...
                  <span class="jdt-days">{{
                    $t("Market_23").replace(
                      "#0#",
                      this.getNowPetItem.durationDays
                    )
                  }}</span>
                  <p class="jdt-bg">
                    <span
                  ...
                  <span
                    class="jdt-bar"
                    :class="this.getWidth == '100%' ? 'active' : ''"
                  ...
                  <span class="jdt-startPrice vertical-children">
                    ...
                    />&nbsp;
                    <span>{{
                      numFloor(this.getNowPetItem.startPrice / 1e9, 1e4)
                    }}</span>
                  </span>
                  <span class="jdt-endPrice vertical-children">
                    ...
                    />&nbsp;
                    <span>{{
                      numFloor(this.getNowPetItem.endPrice / 1e9, 1e4)
                    }}</span>
                  </span>
                </div>
                <p
                  class="small vertical-children por"
                  style="top: -15px"
                  v-if="this.nowPrice != this.getNowPetItem.endPrice"
                >
              ...
              <div v-if="!isMyPet" class="mgt-30">
                <div :class="{ 'btn-group': needApprove }" class="dib">
                  ...
                  <div class="mgt-10">
                    <StatuButton
                      ...
                      :isDisable="
                        needApprove || nowTs - this.getNowPetItem.uptime <= 120
                      "
                      :onClick="() => oprDialog('confirm-buy-dialog', 'block')"
                    >
                      <template v-if="nowTs - this.getNowPetItem.uptime <= 120">
                        ...
                        <span>{{
                          getLeftTime(
                            Number(this.getNowPetItem.uptime) + 120 - nowTs
                          )
                        }}</span>
                      </template>
                ...
                <template
                  v-if="
                    !needApprove &&
                    nowTs - this.getNowPetItem.uptime > 120 &&
                    isMatchShopCar
                  "
                >
                  <button
                    ...
                    style="width: 150px"
                    class="mgt-10 mgl-10"
                    :class="isInShopCar ? 'btn-danger' : 'btn-line'"
                  >
              ...
              <div v-if="isMyPet" class="mgt-20">
                <button
                  class="btn-primary vertical-children por"
                  :class="lockBtn.changePriceLock > 0 ? 'disable-btn' : ''"
                  @click="setChangePriceData(true)"
                >
                  <Loading
                    class="btn-loading"
                    v-if="lockBtn.changePriceLock > 0"
                  />
                  {{ $t("Market_20") }}</button
                >&nbsp;&nbsp;
                <button
    ...
    <Dialog id="changePrice-dialog" :top="200" :width="400">
      ...
      >
        {{
          $t("Market_14")
            .replace("#0#", sellObj.durationDays)
      ...
      <button
        ...
        :class="
          !(sellObj.durationDays > 1 && sellObj.startPrice > 0)
            ? 'disable-btn'
            : ''
        "
        @click="changePrice"
    ...
    <Dialog id="confirm-buy-dialog" :top="200" :width="350">
      ...
      <div class="mgt-50">
        <button
          class="btn-primary"
          @click="oprDialog('confirm-buy-dialog', 'none')"
        >
          {{ $t("Common_04") }}
        </button>
        <button
          ...
          @click="
            oprDialog('confirm-buy-dialog', 'none');
            buyPet();
          "
        >
          {{ $t("Common_03") }}
  ...
  computed: {
    ...
    message() {
      ...
      return this.$t("Market_102")
        .replace(
          "#0#",
          `<span style='color: #49c773' class='money'>${this.standardsHashrate}</span>`
        )
        .replace(
          "#1#",
          `<span style='color: #49c773' class='money'>${this.numFloor(
    ...
    isMatchShopCar() {
      return (
        this.getNowPetItem.tokenId != 0 ||
        (this.getShowList.length == 1 && Number(this.getShowList[0].num) <= 1)
      );
    },
    //
    isMeetStandards() {
    ...
    needApprove() {
      return (
        this.coinArr["USDT"].allowanceToAuction / 1e18 <
          Number(this.nowPrice / 1e9) &&
        this.coinArr["USDT"].allowanceToAuction != -1
      );
    },
  },
  async created() {
  ...
  methods: {
    ...
    setNowPrice() {
      ...
      if (endTime > nowTime) {
        nowPrice =
          Number(startPrice) +
          (diffPrice / durationDays) * Math.floor((nowTime - uptime) / 86400);
        this.getWidth =
          ((nowTime - uptime) / (durationDays * 86400)) * 100 + "%";
      } else {
        this.getWidth = "100%";
      }
      ...
      if (endTime > nextDayTime) {
        nextDayPrice =
          startPrice +
          (diffPrice / durationDays) *
            Math.floor((nextDayTime - uptime) / 86400);
      }
    ...
    async cancelAuction() {
      ...
      if (hash) {
        //
        this.tempMarketCancelTx.push({
          tx: this.getNowPetItem.tx,
          nextHash: hash,
        });
    ...
    async changePrice() {
      ...
      if (endPrice == "" || this.priceTypePos == 0)
        this.sellObj.endPrice = startPrice;

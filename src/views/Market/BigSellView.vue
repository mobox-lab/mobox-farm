<template>
    <div id="big-sell-view" style="margin-top:10px">
        ...
        <div class="tac row mgt-10">
			...
			<div class="col-md-6">
				<div class="panel tal" >
                    <p  v-if="hasOwn" style="color:red" class="tar">*{{$t("Market_99")}}</p>
                    <div v-for="item in getShowList" :key="item.prototype" :class="'pet_hover_lv' +getVType(item.prototype)" class="shop-car-item  vertical-children por mgt-10 por">
		                <img v-if="!isMeetStandards(item)" src="@/assets/icon/warning-icon.png" class="tip-icon" @click="standardsHashrateTip(item)" />
                        <div class="dib por">
                             <p class="vertical-children small momo-tab-hash" v-if="item.level > 1 && getNowPetItem.type == 0" style="transform: translateX(-50%);left:50%;z-index:99;position:absolute;top:5px;background:rgba(0,0,0,0.5);border-radius:30px;padding:0px 5px;white-space: nowrap">
                                <img src="@/assets/icon/airdrop.png" width="10" alt=""  />&nbsp;
                                <span  :class="getHashrateColor(item)">{{item.hashrate}}</span>
                            </p>
                            <img  :src="require(`@/assets/pet/${item.prototype}.png`)" alt="" width="100" height="100" v-if="getNowPetItem.type == 0" />
                        </div>
                        <div class="dib mgl-5" style="width:120px;padding:10px" v-if="getNowPetItem.type == 1">
                            <div class="momo-tab momo-tab-min-type4 dib por" style="margin:3px">
                                <p class="vertical-children small momo-tab-hash" v-if="item.level > 1" style="transform: translateX(-50%);left:50%;trz-index:99;position:absolute;top:-16px;background:rgba(0,0,0,0.5);border-radius:30px;padding:0px 5px;white-space: nowrap">
                                    <img src="@/assets/icon/airdrop.png" width="10" alt=""  />&nbsp;
                                    <span  :class="getHashrateColor(item)">{{item.hashrate}}</span>
                                </p>
                                <img class="momo-tab-img" :src="require(`@/assets/pet/${item.prototype}.png`)" alt="" width="100%" />
                            </div>
                            <div class="momo-tab dib" style="margin:3px" v-for="item2 in item.show_1155" :key="item2"  :class="`momo-tab-min-type`+ parseInt(item2 / 1e4)">
                        ...
                        <!-- 宝石 -->
                        <div class="jewel" >
                            <div v-for="(dPos, index) in [100,200,300,400 ]" :key="JSON.stringify(''+dPos+item.tokenId)" class="gem-item-little">
                                <img v-if="momoGemsObjs[item.tokenId] == undefined || momoGemsObjs[item.tokenId][index] == 0" :src="require(`@/assets/market/${dPos+1}.png`)" class="opa-3 gray" alt="" title="jewel"/>
                                <img v-else :src="require(`@/assets/market/${momoGemsObjs[item.tokenId][index]}.png`)" alt="">
                            </div>
                        </div>
                         <!-- /宝石 -->
                         <!-- 是否已拥有 -->
                ...
                <div class="panel vertical-children">
                    ...
                    <div class="tac">
                        <div id="price" class="vertical-children">
                            <img :src="require(`@/assets/coin/USDT.png`)" height="25" alt="">&nbsp;
                            <span class="money" >{{numFloor( this.getNowPetItem.price/ 1e9, 1e2).toLocaleString()}}</span>
                        </div>
                    </div>
                    <div  v-if="isMyPet" class="mgt-20 tac">
                        <StatuButton  :onClick="setChangePriceData" :isLoading="lockBtn.changePriceLock > 0">{{$t("Market_20")}}</StatuButton>&nbsp;&nbsp;
                    ...
                    <div v-else class="mgt-20 tac">
                        <div :class="{'btn-group': needApprove}" class="dib">
                            ...
                            <div class="mgt-10">
                                <StatuButton style="width:150px"  data-step="2" :isLoading="lockBtn.buyMomoLock > 0" :isDisable="needApprove || (nowTs - this.getNowPetItem.uptime) <= 600" :onClick="()=>oprDialog('confirm-big-buy-dialog','block')">
                ...
                <div class="panel mgt-20" v-if="!isMyPet">
                    ...
                    <div class="aveage-box upgrade-item" v-if="getAddObj.static.dh > 0">
                        ...
                        <div class="vertical-children bold2 tar">
                            ...
                            <span class="color-sell" >(+{{fixedHashrate.after - fixedHashrate.current}})</span>
                        </div>
                    </div>
                    <div class="aveage-box upgrade-item" v-if="getAddObj.percent.dh != '0%' ">
                        <p>{{$t("Market_91")}}</p>
                        <div class="vertical-children bold2 tar">
                            <span>{{getAddObj.percent.now}}</span>
                            <img src="@/assets/icon/upgradejt.png" alt="" class="mgl-5" height="10"/>
                            <span class="mgl-5" >{{isAllMeetStandard ? getAddObj.percent.to : 0}} </span>
                            <span class="color-sell" >(+{{isAllMeetStandard ? getAddObj.percent.dh : '0%'}})</span>
                        </div>
                    </div>
                    <div class="aveage-box upgrade-item" >
                        ...
                        <div class="vertical-children bold2 tar">
                            ...
                            <img src="@/assets/icon/upgradejt.png" alt="" class="mgl-5" height="10"/>
                            <span class="mgl-5" >{{isAllMeetStandard ? getAddObj.total.to : 0}} </span>
                            <span class="color-sell" v-if="getAddObj.total.dh > 0">(+{{isAllMeetStandard ? getAddObj.total.dh : 0}})</span>
                        </div>
                    </div>
                    <div class="aveage-box upgrade-item" >
                        <p>{{$t("Market_93")}}</p>
                        <div class="vertical-children bold2 tar">
                            ...
                            <img src="@/assets/coin/USDT.png" alt="" class="mgl-5" height="20"/>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mgt-20">
                <div class="ly-input-content mgt-10">
					...
					<div class="por mgt-5">
						...
						<input v-model="inputPrice"   class="ly-input sell-input" type="text" :placeholder="$t('Market_17')" v-number  data-max="100000000"/>
					</div>
				</div>
            </div>
            <button style="width: 200px" class="btn-primary mgt-20" :class="{'disable-btn': inputPrice == ''}" @click="changePrice">
				{{$t("Common_03")}}
			</button>
        </Dialog>
        <!-- 确认购买弹窗 -->
        <Dialog id="confirm-big-buy-dialog"  :top="200" :width="350">
			...
			<div class="mgt-50">
				<button class="btn-default" @click="oprDialog('confirm-big-buy-dialog', 'none');">{{$t("Common_04")}}</button>
				<button class="btn-primary mgl-5" @click="oprDialog('confirm-big-buy-dialog', 'none');buyPet()">{{$t("Common_03")}}</button>
			</div>
		</Dialog>
    </div>
</template>
    ...
    computed:{
		...
		message() {
			...
			if (status.indexOf(false) === -1) {
				return this.$t('Market_59').replace('#0#', `<span style='color: #49c773' class='money'>${this.numFloor(this.getNowPetItem.price/ 1e9, 1e2).toLocaleString()} USDT</span>`);
			}
			...
			return this.$t('Market_103')
				.replace('#0#', `<span style='color: #49c773' class='money'>${standardsHashrate}</span>` )
				.replace('#1#', `<span style='color: #49c773' class='money'>${this.numFloor(this.getNowPetItem.price/ 1e9, 1e2).toLocaleString()} USDT</span>`)
		},
        getAddObj(){
            let obj = {
            ...
            //设置百分比加成
            obj.percent.now = this.numFloor(Common.app.getTotalPercent.maxAdd * 100, 100) + "%";
            obj.percent.to = this.numFloor(this.getNewTotalPercent.maxAdd * 100, 100) + "%";
            obj.percent.dh =  this.numFloor((this.getNewTotalPercent.maxAdd- Common.app.getTotalPercent.maxAdd) * 100, 100) + "%"
            //设置总算力
            obj.total.to = this.getNewTotalPower;
            obj.total.dh = parseInt(this.getNewTotalPower - obj.total.now);
            obj.perPrice = this.numFloor(this.numFloor( this.getNowPetItem.price/ 1e9, 1e2) / obj.total.dh, 1e2);
            return obj;
        },
        getNewPowerUpList(){
            let showList = [];
            ...
            if(this.getNowPetItem.type == 1){
                this.getShowList.map(item=>{
                    item.show_1155.map(pType=>{
                        addMomo.push({prototype: pType, num: 1, lvHashrate: this.getVType(pType)})
                    })
                })
            }
        ...
        getNewTotalPower(){
            ...
            totalHash = (totalHash + this.getNewPowerUpList.length * 300) * (1+this.getNewTotalPercent.maxAdd)
        ...
        getOldPower(){
            ...
            totalHash = (totalHash + Common.app.showPowerUpList.length * 300) * (1+Common.app.getTotalPercent.maxAdd)
            return parseInt(totalHash);
        },
        getNftVInfo() {
			let retObj = {
        ...
        getShowList(){
            ...
            tokens.map(item=>{
                ...
                item.hasOwn = this.getCompProto.indexOf(item.prototype.toString()) != -1;
                item.num = 1;
        ...
        //是否需要授权, 授权额度不足支付
		needApprove(){
			return this.coinArr['USDT'].allowanceToBigAuction / 1e18 < Number(this.getNowPetItem.price/ 1e9) && this.coinArr['USDT'].allowanceToBigAuction !=-1;
		},
        // 固定算力
        fixedHashrate() {
            const momos = [...this.myNFT_wallet,...this.myNFT_verse,...this.myNFT_stake].filter((item) => {
    ...
    methods: {
        ...
        analyzeMyMomos(myMomos) {
            let myMomoData = {
                ...
                v1: {},
                v1Amount: 0,
                fixedHashrate: 0,       // 加成前算力
                v4Group: {},            // 紫色组合[prototype] = 1
                v4GroupAmount: 0        // 紫色组合数量
            }
        ...
        async viewAllowance(){
			...
			let allowanceToBigAuction = await Wallet.ETH.viewErcAllowanceToTarget(PancakeConfig.SelectCoin[coinKey].addr, WalletConfig.ETH.bigSell, false);
			if(allowanceToBigAuction){
				this.coinArr[coinKey].allowanceToBigAuction = Number(allowanceToBigAuction);
		...
		async approve(){
			...
			WalletConfig.ETH.bigSell, {coinKey, type: "allowanceToBigAuction"});
			if (hash) {
        ...
        async changePrice(){
            ...
            let hash = await Wallet.ETH.Group.BigSell.changePrice(sendData, async ()=>{
        ...
        async cancelAuction(){
            ...
            let hash = await Wallet.ETH.Group.BigSell.cancelAuction( {index_: index, orderId_: orderId}, ()=>{
				this.$router.back(-1);
            });
			if(hash){
				//添加一条临时的下架数据
		...
		async getPetInfo(){
			...
			let data = await Wallet.ETH.Group.BigSell.getOrder({auctor_: auctor, index_: index});
...
.upgrade-item{
    padding:13px 20px;background:#111;border-radius: 10px;
    margin-top: 10px;
    font-size: 15px;
}

<template>
  <!-- 宝石的交易记录 -->
  <Dialog
    id="shop-history-gem-dialog"
    :top="100"
    :width="650"
    :animation="viewHistory"
  >
    <h2>{{ $t("Market_24") }}</h2>
    <div
      class="mgt-10"
      style="background: #000912; border-radius: 10px; padding: 8px"
    >
      ...
      <div style="por mgt-10 tal">
        <div
          class="aveage-box mgt-10 por tal"
          v-for="(item, index) in marketGemHistory.list"
          :key="item.tx + index"
          style="background: #13181f; border-radius: 15px; padding: 10px 8px"
        >
          <div class="shop-history-pet" style="flex: 2; top: -18px">
            <span
              v-for="(item2, index) in item.ids"
              :key="item2 + index"
              style="margin: 0px 2px; zoom: 0.9"
              class="por"
            >
                ...
                <img
                  :src="
                    require(`@/assets/market/${
                      item.erc1155_ == 4 ? item.erc1155_ : item2
                    }.png`)
                  "
                  alt=""
                  height="50"
          ...
          >
            {{ item.isBuy ? "-" : "+"
              ...
              numFloor((item.price / 1e9) * (item.isBuy ? 1 : 0.95), 10000)
            }}
            {{ getMarketCoin(item.crtime) }}
            <p style="color: #8f8f8f; zoom: 0.8">
              {{ dateFtt("yyyy-MM-dd hh:mm:ss", new Date(item.crtime * 1000)) }}
            </p>
          </div>
        </div>
        <div class="no-show" v-if="marketGemHistory.list.length == 0">
  ...
  methods: {
    ...
    async getMyHistory(myAccount) {
      ...
      if (data) {
        ...
        let historyGemNotice =
          newTs > this.erc1155HisTs[this.marketGemFilter] &&
          this.marketGemHistory.uptime != 0;
        //
        if (historyGemNotice) {
          this.erc1155HisTs[this.marketGemFilter] = newTs;
        }
        this.$store.commit("marketState/setData", {
          marketGemHistory: data,
          historyGemNotice,
          erc1155HisTs: this.erc1155HisTs,
        });
      }
    },
  },

  ...
  <div>
    <div class="tal search vertical-children por mgt-20">
      <div id="market-pet-fitter">
        ...
        <div
          v-if="isShowSwapMec"
          class="cur-point dib por mgl-10 mbox-mec-swap"
          @click="openMboxMecSwap"
        >
        ...
        <div class="cur-point dib por mgl-10" @click="toggleShowBulkBuying">
          <span v-if="bulkBuyings.length" class="shop-car-num">{{
            bulkBuyings.length
          }}</span>
        ...
        <div class="dib por mgl-10 filter" v-if="marketTypePos == 2">
          ...
          <div class="filter-panel hide" ref="filter">
            <div>
              <h5>Types</h5>
              <div
                @click="onSelectTypeChange(pos)"
                class="filter-select-item"
                :class="{ active: pos == marketGemSearch.type }"
            ...
            <div class="mgt-20">
              <h5>Levels</h5>
              <div
                @click="onSelectLevelChange(pos)"
                class="filter-select-item"
                :class="{ active: pos == marketGemSearch.level }"
            ...
            <div class="mgt-20">
              ...
              <div
                ...
                class="filter-select-item"
                :class="{ active: pos == marketGemSearch.sort }"
      ...
      <div class="vertical-children dib">
        ...
        <div class="dib filter-show-group" v-if="marketTypePos == 2">
          <div class="filter-show-item" v-if="marketGemSearch.type != 0">
            <span class="filter-close" @click="onSelectTypeChange(0)"
              >&times;</span
            >
            <span class="mgl-10">{{
              $parent.gemType[marketGemSearch.type]
            }}</span>
          </div>
          <div class="filter-show-item" v-if="marketGemSearch.level != 0">
            <span class="filter-close" @click="onSelectLevelChange(0)"
              >&times;</span
            >
            <span class="mgl-10">{{
              $parent.gemLv[marketGemSearch.level]
            }}</span>
          </div>
    ...
    <div
      :class="marketGems.list.length < 4 ? 'tal' : ''"
      class="momo-content vertical-children"
    ...
    >
      ...
      <a
        @click="$router.push({ path: `/auctionGemView/${item.tx}` })"
        v-for="item in marketGems.list"
      ...
      >
        <GemSellItem :key="item.orderId" :data="{ item: item }">
          <div class="vertical-children mgt-10 por" style="font-size: 18px">
            <img
              :src="
                require(`@/assets/coin/${getCurrencyName(item.currency)}.png`)
              "
              alt=""
              height="20"
            />&nbsp;
            <span class="money"
              >{{ numFloor(item.price / 1e9, 100).toLocaleString() }}
              <sub class="small">{{
            ...
            <div class="add-cart" @click.stop="addBulkBuying(item)">
              <img
                v-if="getBulkBuyingIndex(item.tx) == -1"
    ...
    <div style="margin-top: 30px">
      <Page
        ref="page"
        :defaultPage="this.marketGemPage"
        :totalPage="Math.ceil(marketGems.total / onePageCount)"
        :onChange="onPageChange"
        v-show="Math.ceil(marketGems.total / onePageCount) > 1"
      />
    </div>
    <!-- 批量购买购物车 -->
    <BulkBuying
    ...
    >
      <BulkBuyingItem
        ...
        :image="
          require(`@/assets/market/${item.erc1155_ == 4 ? 4 : item.ids[0]}.png`)
        "
        :count="item.amounts[0]"
        :unitPrice="
          numFloor(item.price / item.amounts[0] / 1e9, 1e6).toString()
        "
...
export default {
  ...
  methods: {
    ...
    async getAuctionAll(page, needLoading = false) {
      ...
      let data = await Http.getAuctionListGem(
        "BNB",
        page,
        this.onePageCount,
        this.marketGemSearch
      );
      data.list.map((item) => {
        item.currency = 3;
    ...
    async bulkBuyingPlay(skipSoldOrder) {
      ...
      console.log(amounts);
      const hash = await Wallet.ETH.bidBatch(
        auctors,
        orderIds,
        amounts,
        skipSoldOrder

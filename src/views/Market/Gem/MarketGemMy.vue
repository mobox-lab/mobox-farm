<template>
  <div>
    <div class="tal search vertical-children por mgt-20">
      <div id="market-pet-fitter">
        <div
          ...
          id="shop-car"
          @click="oprDialog('shop-car-dialog', 'block')"
        >
        ...
        <div
          ...
          @click="oprDialog('shop-history-gem-dialog', 'block')"
        >
        ...
        <div class="dib por mgl-10 filter" v-if="marketTypePos == 2">
          ...
          <div class="filter-panel hide" ref="filter">
            <div>
              <h5>Types</h5>
              <div
                @click="fitter.type = pos"
                class="filter-select-item"
                :class="{ active: pos == fitter.type }"
                v-for="(item, pos) in $parent.gemType"
                :key="item"
              >
                {{ item }}
              </div>
            ...
            <div class="mgt-20">
              <h5>Levels</h5>
              <div
                @click="fitter.level = pos"
                class="filter-select-item"
                :class="{ active: pos == fitter.level }"
                v-for="(item, pos) in $parent.gemLv"
                :key="item"
              >
                {{ item }}
              </div>
    ...
    <div
      :class="getMyGem.length < 4 || marketTypePos == 4 ? 'tal' : 'tac'"
      class="momo-content"
    >
      <template v-if="marketTypePos == 2">
        ...
        >
          <div class="tac">
            <SelectNum
              :maxNum="item.num"
              v-show="getSelectNum(item.key) > 0"
              :defaultNum="getSelectNum(item.key)"
              :data="item"
            ...
            <button
              class="btn-primary mgl-5 btn-small"
      ...
      <template v-else>
        <GemSellItem
          v-for="item in get1155SellItems.filter(
            (i) => i.erc1155_ == marketGemFilter
          )"
          :key="item.key"
          :data="{ item: item }"
        >
      ...
      <div class="dialog-content mgt-10" id="shop-car-content">
        ...
        >
          <div class="dib por mgt-10 mgl-10">
            <img
              :src="require(`@/assets/market/${item.key}.png`)"
              alt=""
              height="80"
            />
            <p
          ...
          <div class="absolute-r tar" style="right: 20px; top: 5px">
            ...
            <p class="vertical-children mgt-5">
              <img
                :src="require(`@/assets/coin/${sellCoin}.png`)"
                height="25"
                alt=""
              />
        ...
        <div class="dib tal" style="margin-left: 12px">
          ...
          <p class="vertical-children mgt-5">
            ...
            <span style="font-size: 20px" class="color-w mgl-5"
              ...
              <small style="font-size: 12px">{{ sellCoin }}</small></span
            >
          </p>
          <button
    ...
    <Dialog id="set-price-dialog" :top="200" :width="400">
      ...
      <button
        ...
        :class="`btn-primary mgt-20 ${
          Number(inputPrice) <= 0 ? 'disable-btn' : ''
        }`"
    ...
    <Dialog id="confirm-sell-dialog" :top="200" :width="400">
      ...
      <div class="mgt-10">
        <span
          v-for="item in sellObj.sellData"
          :key="item.key"
          style="
        ...
        >
          <img
            ...
            height="100%"
          />
          <span class="tar" style="position: absolute; right: 3px; bottom: 3px"
            >x{{ item.sellNum }}</span
          >
        </span>
      </div>
      ...
      <p
        class="color-buy tac mgt-10 small"
        v-if="Number(sellObj.startPrice) < 10"
      >
        {{ $t("Market_77") }}
      </p>
      <div
        ...
        :class="{
          'btn-group':
            !hasApprove[needApproveTokenType] &&
            hasApprove[needApproveTokenType] != -1,
        }"
      >
        <StatuButton
          v-if="
          ...
          "
          data-step="1"
          :onClick="approve.bind(this, sellObj)"
        ...
        <StatuButton
          data-step="2"
          class="mgt-10"
          :onClick="confirmSubmit.bind(this)"
          style="width: 70%"
          :isDisable="
            !hasApprove[needApproveTokenType] || Number(sellObj.startPrice) < 10
          "
        >
          {{ $t("Common_03") }}
        </StatuButton>
  ...
  computed: {
    ...
    getMyGem() {
      ...
      for (let key in this.gemBag) {
        ...
        let isMatchType =
          parseInt(Number(key) / 100) == this.fitter.type ||
          this.fitter.type == 0;
        let isMatchLevel =
          Number(key) % 100 == this.fitter.level || this.fitter.level == 0;
  ...
  methods: {
    ...
    async isApprove(item, isForce = false) {
      ...
      let isApprove = await Wallet.ETH.isApprovedForAll(
        this.tokenTypeToAddr[item.erc1155_],
    ...
    confirmSetPrice(event) {
      ...
      if (
        this.shopCar[0] &&
        this.setPriceItem.erc1155_ != this.shopCar[0].erc1155_
      ) {
    ...
    confirmSubmit() {
      ...
      if (sellType == "721") {
        ...
        this.getConfirmDialog().show(
          this.$t("Market_58").replace(
            "#0#",
            `<span style='color: #49c773'>${startPrice} USDT</span>`
          ),
          () => this.confirmSell()
        );
      }

  ...
  <div>
    <div class="tal search vertical-children por mgt-20">
      <div id="market-pet-fitter">
        <div
          class="dib por mgl-10 cur-point"
          ...
          <div class="filter-panel hide" ref="filter">
            <div>
              ...
              <div
                ...
                :class="{ active: pos == myGemMarketSellFilter.type }"
            ...
            <div class="mgt-20">
              ...
              <div
                ...
                class="filter-select-item"
                :class="{ active: pos == myGemMarketSellFilter.level }"
      ...
      <div class="vertical-children dib">
        ...
        <div class="dib filter-show-group" v-if="marketTypePos == 2">
          <div class="filter-show-item" v-if="myGemMarketSellFilter.type != 0">
            ...
            >
            <span class="mgl-10">{{
              $parent.gemType[myGemMarketSellFilter.type]
            }}</span>
          </div>
          <div class="filter-show-item" v-if="myGemMarketSellFilter.level != 0">
            ...
            >
            <span class="mgl-10">{{
              $parent.gemLv[myGemMarketSellFilter.level]
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div
      :class="getShowList.length < 4 ? 'tal' : ''"
      class="mgt-20 vertical-children"
    >
      ...
      <router-link
        :to="item.orderId >= 0 ? '/auctionGemView/' + item.tx : '###'"
        :class="item.orderId >= 0 ? '' : 'opa-6'"
        v-for="item in getShowList"
        :key="item.tx + item.uptime"
      >
        <GemSellItem :key="item.orderId" :data="{ item: item }">
          <template v-if="item.currency == 2">
            ...
            >
              <img
                :src="require(`@/assets/coin/${getShowCoin(item)}.png`)"
                alt=""
                height="20"
              />&nbsp;
              <span
                >{{ numFloor(item.price / 1e9, 10000) }}
                <sub class="small">{{ getShowCoin(item) }}</sub></span
              >
            </div>
            <div
              class="vertical-children mgt-10"
    ...
    <div
      ...
      v-if="Math.ceil(marketGemMy.total / onePageCount) > 1"
    >
      <Page
        ref="page"
        :defaultPage="marketGemMySellPage"
        :totalPage="Math.ceil(marketGemMy.total / onePageCount)"
        :onChange="onPageChange"
        v-show="Math.ceil(marketGemMy.total / onePageCount) > 1"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      myGemMarketSellFilter: (state) =>
        state.marketState.data.myGemMarketSellFilter,
      ...
      marketGemMySellPage: (state) =>
        state.marketState.data.marketGemMySellPage,
  ...
  methods: {
    ...
    async getAuctionPetsMy(needLoading = false) {
      ...
      this.tempGemMarketCancelTx.map((item, index) => {
        if (
          hashArr.indexOf(item.tx) == -1 &&
          item.currency == this.marketGemFilter
        ) {

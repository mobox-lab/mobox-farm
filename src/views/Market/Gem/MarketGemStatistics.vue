      ...
      <div class="aveage-box" style="padding: 30px">
        ...
        <div class="vertical-children tac">
          ...
          <div class="dib mgl-10 statistics-value">
            ...
            <h1 class="color-w tac">
              {{ numFloor(statistics.sevenPrice, 1e2) }}
            </h1>
          </div>
        </div>
      </div>
    ...
    <section class="mgt-20">
      <ul class="statistics-top-tab" style="padding: 0px">
        ...
        <li
          ...
          class="opa-6"
        >
          {{ $t("Market_54") }}({{ tableData.myHistoryTotal }})
        </li>
      </ul>
            ...
            <td class="vertical-children">
              ...
              <span v-else>
                <img
                  class="hide-xs"
                  :src="
                    require(`@/assets/coin/${getCurrencyName(
                      item.currency
                    )}.png`)
                  "
                  height="25"
      ...
      <div style="margin-top: 30px" v-if="tableDataPos == 'myHistory'">
        <Page
          ...
          :totalPage="Math.ceil(marketGemHistory.total / 50)"
          :onChange="onPageChange"
          v-if="Math.ceil(marketGemHistory.total / 50) > 1"
        />
      </div>
    </section>
  </div>
...
export default {
  ...
  methods: {
    ...
    async getMyAuctionHistory() {
      ...
      let data = await Http.getMyGemAuctionHistory(
        myAccount,
        this.myHistoryPage
      );
      this.$store.commit("marketState/setData", { marketLoading: false });

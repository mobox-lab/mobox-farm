<template>
  <div id="market-content" style="padding: 40px 0px 10px 0px">
    <div class="center-content">
      <section id="market-type" class="type-select">
        <span class="type-select-notice" style="right: 5px">&gt;</span>
        <div
          class="type-select-item vertical-children"
          :class="{ active: marketTypePos == 0 }"
          ref="menu0"
          @click="
            $store.commit('marketState/setData', {
              marketTypePos: 0,
              marketTabPos: 0,
            })
          "
        ...
        <div
          class="type-select-item vertical-children"
          :class="{ active: marketTypePos == 4 }"
          ref="menu4"
          @click="
            gemMarketKey = 'box' + Date.now();
            $store.commit('marketState/initGemMarket', {
              defaultSort: 2,
              marketTypePos: 4,
            });
            $store.commit('marketState/setData', {
        ...
        <div
          class="type-select-item vertical-children"
          :class="{ active: marketTypePos == 3 }"
          ref="menu3"
          @click="
            gemMarketKey = 'mecbox' + Date.now();
            $store.commit('marketState/initGemMarket', {
              defaultSort: 2,
              marketTypePos: 3,
            });
        ...
        <div
          class="type-select-item vertical-children"
          :class="{ active: marketTypePos == 5 }"
          ref="menu5"
          @click="
            gemMarketKey = 'mec' + Date.now();
            $store.commit('marketState/initGemMarket', {
              defaultSort: 2,
              marketTypePos: 5,
            });
        ...
        <div
          class="type-select-item vertical-children"
          :class="{ active: marketTypePos == 2 }"
          ref="menu2"
          @click="
            gemMarketKey = 'gem' + Date.now();
            $store.commit('marketState/initGemMarket', {
              defaultSort: 0,
              marketTypePos: 2,
            });
      ...
      <div>
        ...
        <div v-else-if="marketTypePos == 1" class="mgt-10">
          <div class="tal">
            ...
            >
              <Tab
                :list="rentTabList"
                :defaultSelectPos="marketTabPos"
                :onChange="onTabChange"
                :notice="[0, 0, marketRentOrderList.total]"
              />
            </div>
        ...
        <!-- 1155市场 -->
        <div
          v-else-if="[2, 3, 4, 5].indexOf(marketTypePos) != -1"
          class="mgt-10"
          :key="gemMarketKey"
        >
          <div class="tal">
          ...
          <div style="padding: 0px 10px">
            <Swap
              v-show="
                marketTabPos == 0 && marketTypePos == 5 && !isShowMecMarket
              "
            />
            <MarketGemAll
              v-show="
                marketTabPos == 0 &&
                (marketTypePos == 5 ? isShowMecMarket : true)
              "
              :isShowSwapMec="marketTypePos === 5"
            />
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      marketRentOrderList: (state) =>
        state.marketState.data.marketRentOrderList,
    ...
    getSelectCoinArr() {
      ...
      Object.keys(PancakeConfig.SelectCoin).map((coinKey) => {
        if (coinKey == "USDT") {
          arr.push(
            coinKey + ": " + this.numFloor(this.coinArr[coinKey].balance, 1e2)
          );
        }
      });
      return arr;

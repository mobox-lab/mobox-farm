    ...
    <section>
        ...
        <Dialog id="big-sell-dialog" :top="20" :width="570">
            ...
            <div class="tal mgt-10">
                <div @click="sellType =  0"  :class="sellType == 0?'active':''" class="tab-menu " >{{$t('Market_80')}}</div>
                <div @click="sellType =  1"  :class="sellType == 1?'active':''"  class="tab-menu"  >{{$t("Market_81")}}</div>
            </div>
            <div class="tab-panel" style="background: #1f242c;padding:20px">
                ...
                <button class="btn-primary mgt-30" style="min-width: 160px" :class="{'disable-btn': getTotalSelectNum < 3}" @click="goSetPrice">{{$t("Market_84")}}</button>
            </div>
        </Dialog>
        <!-- 出售价格预览 -->
        <Dialog id="big-sell-set-price-dialog" :top="20" :width="570">
              ...
              <div class="por tal mgt-10"> 
				...
				<p class="cur-point vertical-children" style="position: absolute; right: 0px; top: 0px" @click="initSelectData()" >
            ...
            <div class="dialog-content momo-c mgt-10 tal">
                <div v-for="item in selectItems" :key="item.prototype+item.tokenId" :class="'pet_hover_lv' +item.vType" class="shop-car-item  vertical-children por mgt-10 por">
                    <div class="dib por">
                        ...
                        <img style="position:absolute; bottom:10px;left:10px" v-if="item.hasGem" src="@/assets/icon/hasgem.png" height="20" alt="" />
                     ...
                     <div class="dib mgl-5" style="zoom: 0.7" v-if="sellType == 1">
                        <div class="momo-tab" v-for="item2 in item.show_1155" :key="item2"  :class="`pet-min-type`+ parseInt(item2 / 1e4)">
                    ...
                    <div class="absolute-r tar" style="right: 10px; top:auto; bottom:20px;">
						...
						<img @click="editPrice(item)" class="mgl-5 cur-point" src="@/assets/icon/edit.png" height="24" alt="" />
					</div>
                    <div @click="onSelectChange(item)" class="absolute-r  vertical-children cur-point" style="background: rgba(0,0,0,0.3); border-radius: 0px 14px 0px 14px;right: 0px;padding:3px 15px">
            ...
            <div class="vertical-children mgt-20 tal">
                ...
                <h3 class="mgl-5 bold2 dib" >{{getTotalPrice.toLocaleString()}} <sub class="opa-6">USDT</sub> </h3>
            </div>
            <div class="mgt-30">
                <button class="btn-default" @click="oprDialog('big-sell-dialog','block');oprDialog('big-sell-set-price-dialog','none');">{{$t("Market_86")}}</button>
                <button class="btn-primary mgl-20" @click="checkProps()">{{$t("Market_87")}}({{selectItems.length-unSetPriceCount}}/{{selectItems.length}})</button>
            </div>
        ...
        <Dialog id="set-one-price-dialog" :top="20" :width="400">
            ...
            <span style="zoom: 0.7" v-if="setPriceItem.tokenId != undefined">
                <PetItem style="margin:10px 0px"  v-bind:data="{item: setPriceItem}" ></PetItem>
            </span>
            <div class="mgt-10">
                <div class="ly-input-content mgt-10">
                    ...
                    <div class="por mgt-5">
                        ...
                        <input v-model="inputPrice" style="width:100%"   class="ly-input tac" type="text" :placeholder="$t('Market_17')" v-number  data-max="100000000"/>
                    </div>
                </div>
            </div>
            <button style="width: 200px" class="btn-primary mgt-20" :class="{'disable-btn': Number(inputPrice) <=0 }" @click="confirmSetPrice">
                {{$t("Common_03")}} 
            </button>
        </Dialog>
        <!-- 重复确认出售总价 -->
        <Dialog id="re-input-price-dialog" :top="20" :width="400">
            ...
            <h4 class="mgt-20" v-html="$t('Market_58').replace('#0#', `<span style='color: #49c773'>${getTotalPrice.toLocaleString()} USDT</span>` )"></h4>
            <input v-model="reCheckPrice" style="width:100%"   class="ly-input tac mgt-20" type="text" :placeholder="$t('Market_88')" v-number  data-max="100000000"/>
            <div class="mgt-30">
				<button class="btn-default" @click="oprDialog('re-input-price-dialog', 'none');">{{$t("Common_04")}}</button>
				<button class="btn-primary mgl-20" @click="confirmSell()">{{$t("Common_03")}}</button>
			</div>
        </Dialog>
    </section>
</template>
...
export default{
    ...
    computed: {
        ...
        getShowMoMoList(){
            if(this.sellType == 0){
                return this.myNFT_stake.filter(momo => momo.vType >= 4).sort((a,b)=>(b.lvHashrate - a.lvHashrate) * this.sort);
            }else{
                let arr = [];
        ...
        selectItems(){
            let {v4_tokenId, v5_tokenId, v6_tokenId, v4_price, v5_price, v6_price, upTs} = this.selectProtoTypes;
            let arr = [];
            ...
            allTokenId.map((item, pos)=>{
                ...
                arr.push({...this.momoData[item], sellPrice: allPrice[pos], upTs, show_1155});
            });
            return arr;
        },
        getTotalPrice(){
    ...
    methods:{
        ...
        confirmSetPrice(){
            ...
            let pos = this.selectProtoTypes["v"+vType+"_tokenId"].indexOf(tokenId);
            this.selectProtoTypes["v"+vType+"_price"][pos] = Number(this.inputPrice);
        ...
        async confirmSell(){
            ...
            let tokenIds = [...v4_tokenId, ...v5_tokenId, ...v6_tokenId].sort((a, b)=>a-b);

            if(tokenIds.length < 3) return;
            
            let obj = {suggestIndex_, groupType: this.sellType, tokenIds, price_: this.getTotalPrice};
            console.log(obj);

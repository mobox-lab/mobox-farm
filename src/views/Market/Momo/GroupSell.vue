    ...
    <Dialog id="group-sell-dialog" :top="20" :width="500">
      ...
      <div class="por tal mgt-10">
        ...
        <p
          ...
          @click="$store.commit('marketState/emptyGroupSellCar')"
        >
      ...
      <div class="vertical-children por mgt-10 tac">
        <button
          ...
          :class="{ 'disable-btn': groupSellCar.length == 0 }"
          class="btn-primary"
        >
          {{ $t("Market_66") }}
        </button>
  ...
  methods: {
    ...
    async addTempMomo(auctionObj, group721Data, hash) {
      ...
      ids_.map((item, pos) => {
        ...
        tempDate.tokenId = 0;
        tempDate.ids = [item];
        (tempDate.amounts = [1]), (tempDate.endPrice = prices1155_[pos] * 1e18);

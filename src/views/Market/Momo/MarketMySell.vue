  ...
  <div>
    <div class="tal search vertical-children por mgt-20">
          ...
          <div class="filter-panel hide" ref="filter">
            <div>
              ...
              <div
                ...
                :class="{ active: pos == myMarketSellFilter.vType }"
      ...
      <div class="vertical-children dib">
        <span
          >{{ $t("Market_33") }}({{
            marketPetsMy.total + bigSellMy.total
          }})</span
        >
        <div class="dib filter-show-group">
          <div class="filter-show-item" v-if="myMarketSellFilter.vType != 0">
            ...
            >
            <span class="mgl-10">{{
              $parent.$parent.selectVType[myMarketSellFilter.vType]
            }}</span>
          </div>
        </div>
      </div>
    ...
    >
      <div
        class="no-show"
        v-if="getShowList.length == 0 && getBigShowList.length == 0"
      >
      ...
      <div>
        <router-link
          :to="`/bigSellView/` + item.tx"
          v-for="item in getBigShowList"
          :key="item.tx"
        >
      ...
      <div>
        <router-link
          :to="item.index >= 0 ? '/auctionView/' + item.tx : '###'"
          :class="item.index >= 0 ? '' : 'opa-6'"
          v-for="item in getShowList"
          :key="item.tx + item.uptime + item.tokenId + item.ids[0]"
        >
          <PetItem
            v-bind:data="{ item: item.tokenId == 0 ? item.list1155[0] : item }"
            class="market"
            v-if="item.tokenId != 0 || item.list1155.length == 1"
          >
            <div
            ...
            >
              ...
              <span
                >{{ numFloor(item.nowPrice / 1e9, 10000) }}
                <sub class="small">BUSD</sub></span
              >
            </div>
            <div
              class="vertical-children"
    ...
    <div
      ...
      v-if="Math.ceil(marketPetsMy.total / onePageCount) > 1"
    >
      <Page
        ref="page"
        :defaultPage="marketMySellPage"
        :totalPage="Math.ceil(marketPetsMy.total / onePageCount)"
        :onChange="onPageChange"
        v-show="Math.ceil(marketPetsMy.total / onePageCount) > 1"
      />
    </div>
  </div>
</template>
...
export default {
      ...
      this.marketPetsMy.list.map((item) => {
        ...
        if (item.ids.length != 0) {
          item.ids.map((prototype) => {
            ...
            if (this.myMarketSellFilter.category == category)
  ...
  methods: {
    ...
    async getMomoName(needGetNameArr) {
      ...
      this.$store.commit("marketState/setData", {
        marketPetsMy: this.marketPetsMy,

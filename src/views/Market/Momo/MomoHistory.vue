    ...
    >
      ...
      <div style="por mgt-10 tal">
        <div
          ...
          v-for="(item, index) in marketHistory.list"
          :key="item.tx + index"
        ...
        >
          <div class="shop-history-pet" style="flex: 2">
            <span
              ...
              :key="'' + item2.prototype + item2.tokenId"
            ...
            >
              <PetItemMin
                :data="item2"
                style="zoom: 0.5"
                :isTop="index > marketHistory.list.length - 4"
              />
            </span>
          </div>
          ...
          >
            {{ item.isBuy ? "-" : "+"
              ...
              numFloor((item.bidPrice / 1e9) * (item.isBuy ? 1 : 0.95), 10000)
            }}
            BUSD
  ...
  methods: {
    ...
    async getMyHistory(myAccount) {
      ...
      if (data) {
        ...
        if (!historyNotice) {
          historyNotice =
            data.total != this.marketHistory.total &&
            this.marketHistory.uptime != 0;
        }

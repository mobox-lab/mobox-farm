<template>
    <div class="big-sell-item" :class="{'opa-6': nowTs -data.uptime <=  600}">
        <div class="big-sell-show aveage-box">
            <div v-for="pos in 3" :key="getTokens[pos-1].tokenId" class="big-sell-show-item" :class="`pet_hover_lv`+getVType(getTokens[pos-1].prototype)">
		        <img v-if="!isMeetStandards(pos - 1)" src="@/assets/icon/warning-icon.png" class="tip-icon" @click.stop="standardsHashrateTip(pos - 1)" />
               ...
               <div class="vertical-children ">
                   ...
                   <span class="mgl-5" :class="getHashrateColor(getTokens[pos-1])">{{getTokens[pos-1].lvHashrate}}</span>
               </div>
               <div class="small opa-6 hide-xs" v-if="getTokens[pos-1].level > 1">
                   <span>Lv.1</span>
                    <img class="mgl-5" src="@/assets/icon/airdrop.png" alt="" height="10">&nbsp;
                    <span  :class="getHashrateColor(getTokens[pos-1])">{{getTokens[pos-1].hashrate}}</span>
               </div>
            </div>
        ...
        <div class="aveage-box" style="padding: 20px">
            ...
            <div class="tac vertical-children">
                ...
                <span class="mgl-5 money" style="font-size: 18px">{{numFloor(data.price / 1e9, 1e2).toLocaleString()}}<span class="small"> USDT</span></span>
            </div>
        </div>
        <div v-if="nowTs -data.uptime <=  600" class=" mgt-10 small up-time" >
            <p class="small">{{$t("Market_30")}}: {{getLeftTime(Number(data.uptime)+600- nowTs)}}</p>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        getTokens(){
            ...
            this.data.tokens.map(item=>{
                tokens.push({...NftCfg[item.prototype], ...item, noHover: true, isGroup: this.data.type == 1})
            })
    ...
    methods: {
		...
		standardsHashrateTip(index) {
            this.getConfirmDialog().show(`${this.$t('MOMO_98').replace('#0#', this.standardsHashrate(index)).replace('#0#', this.standardsHashrate(index))}`);
		},
		// 是否不满足标准算力

			...
			<section class="statistics-top">
				<div>
					<ul class="statistics-top-tab">
						<li @click="daySelect = 1;getStatistics() " :class="daySelect == 1?'active':''" class="opa-6">{{$t("Market_41")}}</li>
						<li @click="daySelect = 7;getStatistics() " :class="daySelect == 7?'active':''" class="opa-6">{{$t("Market_42")}}</li>
						<li @click="daySelect = 30;getStatistics() " :class="daySelect == 30?'active':''" class="opa-6">{{$t("Market_43")}}</li>
						<li @click="daySelect = 'all';getStatistics() " :class="daySelect == 'all'?'active':''" class="opa-6">{{$t("Market_61")}}</li>
					</ul>
				...
				<div class="aveage-box" style="padding:30px">
					...
					<div class="vertical-children tac">
						...
						<div class="dib mgl-10 statistics-value">
							...
							<h1 class="color-w tac">{{numFloor(statistics.avgPrice / 1e6, 1e2)}}</h1>
						</div>
					</div>
				</div>
			</section>
...
export default {
	...
	methods: {
		...
		async getStatistics(){
			...
			let res = await Erc1155TradeHttp.post("/erc1155/markets/data", {days: this.daySelect});

<template>
  <section>
    <Dialog id="rent-deal-dialog" :top="150" :width="400">
      <h2>{{ $t("Hire_33") }}</h2>
      <p class="mgt-10" v-if="isRentOther">
        {{ $t("Hire_10") }}: {{ shorAddress(statusObj.owner) }}
      </p>
      <p class="mgt-10" v-else>
        {{ $t("Hire_09") }}: {{ shorAddress(statusObj.renter) }}
      </p>
      <div class="ly-input-content mgt-10">
        <div class="aveage-box tal mgt-20">
          ...
          <p class="tar vertical-children" v-if="statusObj.orderId != '-'">
            <img
              :src="require(`@/assets/coin/${showCoin}.png`)"
              alt=""
              height="25"
            />
            <span class="mgl-5">
              <span v-if="statusObj.rentPrice != '-'"
                >{{ numFloor(statusObj.rentPrice / 1e18, 1e4) }}
                {{ showCoin }}</span
              >
        ...
        <div class="aveage-box tal mgt-10">
          <p>{{ $t("Hire_56") }}</p>
          <p class="tar">
            <span v-if="statusObj.rentDays != '-'"
              >{{ statusObj.rentDays }} {{ $t("Hire_46") }}</span
            >
            <span v-else><Loading /></span>
          </p>
        </div>
        ...
        <StatuButton
          :onClick="() => oprDialog('rerent-day-dialog', 'block')"
          :isLoading="lockBtn.rentLock > 0"
          class="mgt-20"
          v-if="Number(statusObj.rentDays) > 0 && isRentOther"
          :isDisable="Number(statusObj.leftTs) <= 0"
        >
    ...
    <Dialog id="rerent-day-dialog" :top="150" :width="390">
      ...
      <section class="mgt-20">
        <p class="vertical-children">
          <span
            >{{ $t("Hire_49") }}:
            {{ numFloor(statusObj.rentPrice / 1e18, 1e4) }} {{ showCoin }}</span
          >
          <img
            :src="require(`@/assets/coin/${showCoin}.png`)"
            alt=""
        ...
        >
          <p class="small tal opa-6">
            {{ $t("Hire_55").replace("#0#", statusObj.rentDays) }}
          </p>
          <div class="por mgt-5">
            <input
          ...
          <div class="aveage-box mgt-10" v-if="Number(inputRentDays) > 0">
            ...
            <p class="tar vertical-children">
              <span
                >{{
                  numFloor((statusObj.rentPrice * inputRentDays) / 1e18, 1e4)
                }}
                {{ showCoin }}</span
              >
              <img
      ...
      <div class="mgt-20">
        <StatuButton
          ...
          :isDisable="
            Number(inputRentDays) <= 0 &&
            Number(inputRentDays) > Number(statusObj.rentDays)
          "
          :isLoading="lockBtn.rentLock > 0"
        >
          {{ $t("Common_03") }}
...
export default {
  ...
  computed: {
    ...
    showCoin() {
      return Number(this.statusObj.orderId) >= 5e4 ? "BUSD" : "MBOX";
    },
  },
  data() {
    return {

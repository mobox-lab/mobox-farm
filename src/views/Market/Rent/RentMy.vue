<template>
  <div>
        ...
        <div class="dib por mgl-10 filter">
          ...
          <div class="filter-panel hide" ref="filter">
            <div>
              ...
              <div
                ...
                :class="{
                  active:
                    pos ==
                    (myRentMarketFilter.vType == 0
                      ? 0
                      : myRentMarketFilter.vType - 3),
                }"
            ...
            <div class="mgt-20">
              <h5>Status</h5>
              <div
                @click="onSelectState(pos)"
                class="filter-select-item"
                :class="{ active: pos == myRentMarketFilter.state }"
                v-for="(item, pos) in momoState"
                :key="item"
              >
                {{ item }}
              </div>
      ...
      <div class="vertical-children dib">
        ...
        <div class="dib filter-show-group">
          <div class="filter-show-item" v-if="myRentMarketFilter.vType != 0">
            ...
            <span class="mgl-10">{{
              selectVType[
                myRentMarketFilter.vType == 0 ? 0 : myRentMarketFilter.vType - 3
              ]
            }}</span>
          </div>
          <div class="filter-show-item" v-if="myRentMarketFilter.state != 0">
            ...
            <span class="mgl-10">{{
              momoState[myRentMarketFilter.state]
            }}</span>
          </div>
        </div>
      </div>
          ...
          >
            <div v-if="item.vType > 3" class="tac mgt-10">
              <button
                ...
                class="btn-primary btn-small mgt-10"
              >
                {{ $t("Hire_02") }}
              </button>
    ...
    <div
      ...
      v-if="Math.ceil(getTotalPet.length / onePageCount) > 1"
    >
      <Page
        ref="page"
        :defaultPage="marketRentMyPage"
  ...
  computed: {
    ...
    getTotalPet() {
      ...
      [...this.myNFT_stake, ...this.myNFT_verse].map((item) => {
        ...
        let isMatchVType =
          item.vType >= 4 && (vType == 0 || vType == item.vType);
        let rentState = -1;
        if (state == 1) rentState = 0;
        if (state == 2) rentState = 1;
        let isMatchState =
          (item.vType >= 4 && item.rent.state == rentState) || state == 0;

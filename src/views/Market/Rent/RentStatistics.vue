  ...
  <div class="statistics">
    ...
    <section class="mgt-20">
      ...
      >
        ...
        <tbody
          v-for="item in tableData[tableDataPos]"
          :key="item.tx"
          class="por tal"
        >
            ...
            <td class="vertical-children">
              ...
              >
                <img
                  ...
                  :src="
                    require(`@/assets/coin/${
                      Number(item.orderId) >= 5e4 ? 'BUSD' : 'MBOX'
                    }.png`)
                  "
                  alt=""
                  height="25"
                ...
                <span v-if="Number(item.orderId) >= 5e4">
                  {{ item.isBuy ? "-" : "+"
                    ...
                    numFloor((item.price / 1e9) * (item.isBuy ? 1 : 0.9), 10000)
                  }}
                  BUSD
                </span>
                <span v-else>
                  {{ item.isBuy ? "-" : "+"
                    ...
                    numFloor(
                      (item.price / 1e9) * (item.isBuy ? 1 : 0.95),
                      10000
                    )
                  }}
                  MBOX
      ...
      <div style="margin-top: 30px" v-if="tableDataPos == 'myHistory'">
        <Page
          ...
          :totalPage="Math.ceil(myHistoryTotal / 50)"
          :onChange="onPageChange"
          v-if="Math.ceil(myHistoryTotal / 50) > 1"
        />
      </div>
    </section>
  </div>
    ...
    async getMyRentHistory() {
      ...
      if (data) {
        data.list.map((item) => {
          ...
          item.isBuy =
            item.renter.toLocaleLowerCase() == myAccount.toLocaleLowerCase();

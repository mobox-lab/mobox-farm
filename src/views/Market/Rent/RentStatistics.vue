		...
		<div class="statistics">
			...
			<section class="mgt-20">
				<ul class="statistics-top-tab" style="padding:0px">
					...
					<li @click="tableDataPos = 'myHistory';getTargetHistory('myHistory')" :class="tableDataPos == 'myHistory'?'active':''" class="opa-6">{{$t("Market_54")}}({{myHistoryTotal}})</li>
				</ul>
				...
				<table class="mgt-20" style="width:100%; border-collapse:collapse;border: none;">
					...
					<tbody v-for="item in tableData[tableDataPos]" :key="item.tx" class="por tal">
						...
						<tr class="list-item">
							...
							<td class="addr">{{getShortAddr(item.renter)}}</td>
							...
							<td class="vertical-children">
								<span :class="item.isBuy?'color-buy':'color-sell'" v-if="tableDataPos == 'myHistory' ">
									<img class="hide-xs"  :src="require(`@/assets/coin/${Number(item.orderId) >= 5e4? getMarketCoin(item.crtime) : 'MBOX'}.png`)" alt="" height="25">
									<span v-if="Number(item.orderId) >= 5e4">
										{{item.isBuy?"-":"+"}}{{numFloor((item.price / 1e9) * (item.isBuy? 1: 0.9), 10000)}} {{ getMarketCoin(item.crtime) }}
									</span>
									<span v-else>
								...
								<span v-else>
									<img class="hide-xs"  :src="require(`@/assets/coin/${Number(item.orderId) >= 5e4 ? getMarketCoin(item.crtime):'MBOX'}.png`)" alt="" height="25">
									{{numFloor(item.price / 1e9, 1e2)}} {{Number(item.orderId) >= 5e4 ? getMarketCoin(item.crtime) : "MBOX" }}
								</span>
							</td>
							<td>{{item.rentDays}}</td>
							<td class="tar" >
				...
				<div style="margin-top: 30px" v-if="tableDataPos == 'myHistory' ">
					<Page :defaultPage="this.myHistoryPage" :totalPage="Math.ceil(myHistoryTotal / 50)" :onChange="onPageChange" v-if="Math.ceil(myHistoryTotal / 50) > 1" />
				</div>
			</section>
		</div>
</template>
...
export default {
	...
	async created(){
		await this.getRentStatistics();
		await this.getRentHistoryAll();
		await this.getMyRentHistory();
	},
	methods: {
		async getTargetHistory(type){
			switch (type) {
				case "recent":
					this.getRentHistoryAll();
					break;
				case "last24":
					this.getLast24();
					break;
		...
		//获取最近记录
		async getRentHistoryAll(){
			this.$store.commit("marketState/setData", {marketLoading: true});
			let res = await Http.getRentHistoryAll();
		...
		async getLast24(){
			this.$store.commit("marketState/setData", {marketLoading: true});
			let res = await Http.getRentRankHistory24();
			this.$store.commit("marketState/setData", {marketLoading: false});
			if(res){
		...
		async getLast(){
			this.$store.commit("marketState/setData", {marketLoading: true});
			let res = await Http.getRentRankHistory();
			this.$store.commit("marketState/setData", {marketLoading: false});
			if(res){
		...
		async getMyRentHistory(){
			...
			let data = await Http.getMyRentHistory(myAccount, this.myHistoryPage);
			this.$store.commit("marketState/setData", {marketLoading: false});
			this.scrollToTop(0);
			if(data){
				data.list.map(item=>{
					...
					item.isBuy =  item.renter.toLocaleLowerCase() == myAccount.toLocaleLowerCase();
				...
				this.myHistoryTotal = data.total;
			}
		},
		scrollToTop(time){
			let t = setTimeout(()=>{
		...
		async getRentStatistics(){
			this.$store.commit("marketState/setData", {marketLoading: true});
			let res = await Http.getRentStatistics(this.daySelect);
			this.$store.commit("marketState/setData", {marketLoading: false});
			if(res){

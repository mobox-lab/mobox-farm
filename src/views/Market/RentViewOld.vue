            ...
            <div id="rent-info-panel">
              <div class="aveage-box">
                ...
                <h3 class="tar vertical-children">
                  ...
                  <span>{{
                    numFloor(statusObj.currentRentPrice / 1e18, 1e4)
                  }}</span>
                </h3>
              </div>
              ...
              <div class="aveage-box">
                <p>{{ $t("Hire_23") }}</p>
                <h3 class="tar">
                  {{ statusObj.currentRentDays }} {{ $t("Hire_46") }}
                </h3>
              </div>
              <div class="tab-split"></div>
              <div class="aveage-box">
                <p>{{ $t("Hire_24") }}</p>
                <h3 class="tar">
                  {{ Number(statusObj.currentRentRound) - 1 }}
                </h3>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    momoState() {
      ...
      if (currentRentDays == "-") return -2; //
      if (currentRentDays == 0) return -1; //
      if (status == 0) {
        return 0; //
      } else {
  ...
  methods: {
    ...
    async rentPet() {
      if (
        this.statusObj.currentRentPrice / 1e18 >
        Number(this.coinArr["MBOX"].balance)
      ) {
    ...
    //
    async cancelRent() {
      let res = await Wallet.ETH.cancelPutRent(
        {
          tokenId_: this.getNowPetItem.tokenId,
          orderId_: this.statusObj.orderId,
        },

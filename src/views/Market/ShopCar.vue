<template>
	<Dialog id="shopcar-dialog" :top="20" :width="500">
		<div class="por tal mgt-10">
			...
			<p class="cur-point vertical-children" style="position: absolute; right: 0px; top: 0px" @click="$store.commit('marketState/emptyShopCar')" >
		...
		<div class="vertical-children por mgt-20 tal aveage-box" >
			<div >
				...
				<div class="dib tal por" style="margin-left: 12px">
					...
					<p class="vertical-children ">
						<img src="@/assets/coin/USDT.png" height="20" alt="" />
						<span style="font-size: 20px" class="color-w mgl-5 money">{{ getShopCarTotalPrice.toLocaleString() }} <small style="font-size:12px">USDT</small></span>
					</p>
				</div>
			</div>

			<div :class="{'btn-group': needApproveUSDT}" class="dib tar" style="padding-right:20px">
				<div v-if="needApproveUSDT">
					<StatuButton data-step="1" :isLoading="coinArr['USDT'].isApproving" :onClick="approve">{{$t("Air-drop_16")}} USDT</StatuButton>
				</div>
				<StatuButton class="mgt-10" data-step="2" :onClick="confirmBuyShopCar" :isDisable="shopCar.length == 0 || needApproveUSDT">
					{{$t("Market_22")}}
				</StatuButton>
			</div>
		</div>
		<div class="mgt-20">
			<div class="tal vertical-children">
				<div class="ly-checkbox" @click="ignoreSold = !ignoreSold" :class="ignoreSold ?'active':'' ">
...
export default {
	...
	computed: {
		...
		needApproveUSDT(){
			return this.coinArr['USDT'].allowanceToAuction < this.getShopCarTotalPrice*1e18;
		}
	},
	methods:{
		confirmBuyShopCar(){
			...
			if (isAllStandard) {
				msg = this.$t('Market_59').replace('#0#', `<span style='color: #49c773'>${this.getShopCarTotalPrice} USDT</span>` );
			} else {
				msg = this.$t('Market_104').replace('#0#', `<span style='color: #49c773'>${this.getShopCarTotalPrice} USDT</span>` );
			}

  ...
  <div id="mypet" style="max-width: 1460px; margin: 40px auto 20px auto">
    ...
    <div class="tal search vertical-children por mgt-10" v-if="tab_pos != 2">
      <div class="vertical-children dib">
        ...
        <div class="dib por powerup">
          <img
            src="@/assets/icon/powerup.png"
            class="cur-point mgl-5"
            alt=""
            height="40"
            @click="oprDialog('showPetPowerUp-dialog', 'block')"
          />
          <span
            v-if="$root.$children[0].showPowerUpList.length <= 0"
            class="pop-notice"
            >{{ $t("MOMO_55") }}</span
          >
        </div>
        ...
        <div class="dib filter-show-group">
          <div class="filter-show-item" v-if="myPetFilter.location != 0">
            <span class="filter-close" @click="setFilter('location', 0)"
              >&times;</span
            >
            <span class="mgl-10">{{
              locationSelect[myPetFilter.location]
            }}</span>
          </div>
          <div class="filter-show-item" v-if="myPetFilter.category != 0">
            <span class="filter-close" @click="setFilter('category', 0)"
              >&times;</span
            >
            <span class="mgl-10">{{
              categorySelect[myPetFilter.category]
            }}</span>
          </div>
          <div class="filter-show-item" v-if="myPetFilter.vType != 0">
            <span class="filter-close" @click="setFilter('vType', 0)"
              >&times;</span
            >
      ...
      <div id="my-pet-fitter" v-if="tab_pos == 0">
        ...
        <div
          ...
          @click="showBatchEnhancement"
          v-if="isShowBatchEnhancement"
        >
        ...
        <div
          class="cur-point dib por mgl-10"
          @click="toggleShowBatchEnhancement"
        >
          <img
            v-if="isShowBatchEnhancement"
            src="@/assets/icon/batch-show-icon.png"
            alt=""
            height="40"
          />
        ...
        <div class="dib por mgl-10 filter">
          ...
          <div class="filter-panel hide" ref="filter">
            ...
            <div class="mgt-10">
              <h5>{{ $t("MOMO_93") }}</h5>
              <div
                @click="setFilter('location', pos)"
                class="filter-select-item"
                :class="{ active: pos == myPetFilter.location }"
                v-for="(item, pos) in locationSelect"
                :key="item"
              >
                {{ item }}
              </div>
            ...
            <div class="mgt-10">
              <h5>Types</h5>
              <div
                @click="setFilter('category', pos)"
                class="filter-select-item"
                :class="{ active: pos == myPetFilter.category }"
            ...
            <div class="mgt-10">
              <h5>Qualities</h5>
              <div
                @click="setFilter('vType', pos)"
                class="filter-select-item"
                :class="{ active: pos == myPetFilter.vType }"
                v-for="(item, pos) in select3"
                :key="item"
              >
                {{ item }}
              </div>
    ...
    <div :class="{ hide: tab_pos != 0 }">
      <div :class="{ tal: getShowPetArr.length < 4 }" class="momo-content">
        <router-link
          :to="
            (item.location == 'auction' ? '/auctionView/' : '/upgrade/') +
            item.prototype +
            '-' +
            item.tokenId +
            '-' +
        ...
        >
          ...
          >
            <div
              class="batch"
              @click="toggleBatch($event, item)"
              v-if="item.vType >= 4 && isShowBatchEnhancement"
            >
              <img
                v-if="batchEnhancementIds.indexOf(item.tokenId) == -1"
                src="@/assets/icon/batch-icon.png"
              />
      ...
      <div
        ...
        v-show="Math.ceil(getTotalPet.length / onePageCount) > 1"
      >
        <Page
          ref="page"
          :defaultPage="myPetPage"
          :totalPage="Math.ceil(getTotalPet.length / onePageCount)"
    ...
    <div v-if="hasShowBook" :class="tab_pos == 1 ? '' : 'hide'">
        ...
        >
          <div style="padding: 10px">
            ...
            <input
              ...
              :value="(getCountMomo.v5 || '-') + '/' + getCountMomo.v5_all"
            />
          </div>
          <div style="padding: 10px">
            ...
            <input
              ...
              readonly
              class="ly-input mgt-10 tac"
              :value="(getCountMomo.v4 || '-') + '/' + getCountMomo.v4_all"
            />
          </div>
          <div style="padding: 10px">
            ...
            <input
              ...
              readonly
              class="ly-input mgt-10 tac"
              :value="$root.$children[0].showPowerUpList.length * 300"
            />
          </div>
          <div style="padding: 10px">
            ...
            <div class="ly-input mgt-10">
              <span class="mark">
                ...
                <img
                  ...
                  @click="oprDialog('standard-hashrate', 'block')"
                />
              </span>
            </div>
          </div>
      ...
      <section id="buy-back" class="mgt-20 por">
        <div class="info">
          <span
            class="cur-point por dib"
            @click="getRootRefs().ruleDialog.show('MOMO_80', 'MOMO_81')"
          >
            <svg
              class="opa-6"
              width="30"
        ...
        >
          <div style="padding: 10px">
            ...
            <div class="ly-input mgt-10 tac vertical-children2">
              {{ getLeftTime(momoSetting.updateTime - nowTs) }}
            </div>
          </div>
          <div style="padding: 10px">
            ...
            <div class="ly-input mgt-10 tac vertical-children2">
              <span
                >[{{ hashrateInfo.v4MinHashrate }},
                {{ hashrateInfo.v4StandardHashrate }},
                {{ hashrateInfo.v4MaxHashrate }}]</span
              >
              <img
                class="mgl-5"
              ...
              <span class="mgl-5"
                >[<span
                  :style="{
                    color:
                      hashrateInfo.v4MinHashrate != nextStepHashrateInfo.v4.min
                        ? 'rgb(133, 243, 74)'
                        : '',
                  }"
                  >{{ nextStepHashrateInfo.v4.min }}</span
                >,
                <span class="color-yellow">{{
                  nextStepHashrateInfo.v4.standard
                }}</span
                >,
                <span style="color: rgb(133, 243, 74)">{{
                  nextStepHashrateInfo.v4.max
                }}</span
                >]</span
              >
            </div>
          ...
          <div style="padding: 10px">
            ...
            <div class="ly-input mgt-10 tac vertical-children2">
              <span
                >[{{ hashrateInfo.v5MinHashrate }},
                {{ hashrateInfo.v5StandardHashrate }},
                {{ hashrateInfo.v5MaxHashrate }}]</span
              >
              <img
                class="mgl-5"
                src="@/assets/icon/upgradejt.png"
              ...
              <span class="mgl-5"
                >[<span
                  :style="{
                    color:
                      hashrateInfo.v5MinHashrate != nextStepHashrateInfo.v5.min
                        ? 'rgb(133, 243, 74)'
                        : '',
                  }"
                  >{{ nextStepHashrateInfo.v5.min }}</span
                >,
                <span class="color-yellow">{{
                  nextStepHashrateInfo.v5.standard
                }}</span
                >,
                <span style="color: rgb(133, 243, 74)">{{
                  nextStepHashrateInfo.v5.max
                }}</span
                >]</span
              >
            </div>
          ...
          <div style="padding: 10px">
            ...
            <div class="ly-input mgt-10 tac vertical-children2">
              [{{ hashrateInfo.v6StandardHashrate }},
              ...
              />
              [<span class="color-yellow">{{
                nextStepHashrateInfo.v6.standard
              }}</span
              >,
              <span class="mgl-5" style="color: rgb(133, 243, 74)">{{
                nextStepHashrateInfo.v6.max
              }}</span
              >]
            </div>
          </div>
      ...
      <div style="overflow: hidden">
        ...
        <div class="row">
          <div
            ...
            v-for="(item, key) in getBookArr.advanced"
            :key="key"
          >
    ...
    <div v-if="tab_pos == 2" class="panel mgt-20" id="preview">
      <div>
        <div class="aveage-box" style="padding: 5px">
          ...
          <div class="tal">
            ...
            <div class="dib mgl-10">
              ...
              <input
                class="ly-input mgt-10"
                type="text"
                style="width: 120px; border-radius: 50px"
                v-model="inputLvHashRate"
                v-int
                :placeholder="
                  inputRange[perviewVTypeSelectPos].min +
                  '~' +
                  inputRange[perviewVTypeSelectPos].max
                "
              />
            </div>
          </div>
        ...
        <div class="mgt-10 visiable-xs">
          <button class="btn-primary" style="width: 50%" @click="search">
            {{ $t("Common_03") }}
          </button>
        </div>
        ...
        <div v-for="item in getTargetLvReport" :key="item.level">
          <div class="aveage-box preview-item">
            ...
            <p class="tal vertical-children">
              ...
              <span class="mgl-10 bold" :class="getHashrateColor(item)">{{
                item.lvHashRate
              }}</span>
            </p>
            <p class="tal">
              <span v-if="item.upgradeCfg.v1 > 0" class="cl-item bg-lv1"
                >x{{ item.upgradeCfg.v1 }}
                <img
                  v-if="(item.level + 1) % 5 == 0"
                  src="@/assets/icon/preview-icon.png"
                  alt=""
                  @click="oprDialog('upgrade-des-dialog', 'block')"
              /></span>
              <span v-if="item.upgradeCfg.v2 > 0" class="cl-item bg-lv2"
                >x{{ item.upgradeCfg.v2
                }}<img
                  v-if="(item.level + 1) % 5 == 0"
                  src="@/assets/icon/preview-icon.png"
                  alt=""
                  @click="oprDialog('upgrade-des-dialog', 'block')"
              /></span>
              <span v-if="item.upgradeCfg.v3 > 0" class="cl-item bg-lv3"
                >x{{ item.upgradeCfg.v3
                }}<img
                  v-if="(item.level + 1) % 5 == 0"
                  src="@/assets/icon/preview-icon.png"
                  alt=""
                  @click="oprDialog('upgrade-des-dialog', 'block')"
              /></span>
              <span v-if="item.upgradeCfg.v4 > 0" class="cl-item bg-lv4"
                >x{{ item.upgradeCfg.v4 }}
                <span class="cl-item-lv" v-if="item.upgradeCfg.v4Lv > 1"
                  >Lv. {{ item.upgradeCfg.v4Lv }}</span
                >
                <img
                  v-if="(item.level + 1) % 5 == 0"
                  src="@/assets/icon/preview-icon.png"
              ...
              <span v-if="item.upgradeCfg.v5 > 0" class="cl-item bg-lv5"
                >x{{ item.upgradeCfg.v5 }}
...
export default {
  ...
  computed: {
    ...mapState({
      batchEnhancementIds: (state) =>
        state.globalState.batchEnhancement.map((item) => item.tokenId),
    ...
    currentTotalAddition() {
      return this.numFloor(
        this.$root.$children[0].currentTotalAddition * 100,
        100
      );
    },
    getNftVInfo() {
    ...
    getTargetLvReport() {
      ...
      for (let index = 1; index <= 40; index++) {
        ...
        if (hashRate >= 50 && hashRate < 80 && this.perviewVTypeSelectPos == 1)
          quality = 5;
    ...
    getTotalPet() {
      ...
      [...this.myNFT_wallet, ...this.myNFT_verse, ...this.myNFT_stake].map(
        (item) => {
          //
          let isMatchCategory =
          ...
          let isMathVType =
            this.myPetFilter.vType == 0 || this.myPetFilter.vType == item.vType;
          let isMatchLocation =
            this.myPetFilter.location == 0 ||
            this.locationName[this.myPetFilter.location] == item.location;
          let isMathProto =
            this.myPetFilter.searchProto <= 0 ||
            item.prototype == this.myPetFilter.searchProto;
          if (
            isMatchCategory &&
            isMathVType &&
            isMathProto &&
    ...
    getShowPetArr() {
      return this.getTotalPet
        .filter(
          (item) =>
            this.myPetFilter.searchProto <= 0 ||
            this.myPetFilter.searchProto == item.prototype
        )
        .slice(
    ...
    getNextHash() {
      ...
      retData.v6 =
        Number(v5_max_upgrade) >= 180 ? Number(v5_max_upgrade) + 30 : 180;
    ...
    nextStepHashrateInfo() {
      ...
      return {
        v4: {
          ...
          standard: Math.floor(
            (Math.floor((step - 8) / 2) * 10 + 10 + ((step - 2) * 10 + 80)) / 2
          ),
          max: (step - 2) * 10 + 80,
        },
        v5: {
          ...
          standard: Math.floor(
            (Math.floor((step - 8) / 2) * 20 + 50 + ((step - 1) * 20 + 150)) / 2
          ),
          max: (step - 1) * 20 + 150,
        },
        v6: {

      ...
      <div class="tab-panel mgt-10" style="background: #1f242c; padding: 20px">
        ...
        <div class="tab-panel momo-c mgt-10 tal">
          <PetItemSmall
            ...
            :isSelected="
              selected.indexOf(item.tokenId) != -1 ||
              selected.indexOf(item.prototype) != -1
            "
          />
        </div>
        ...
        <StatuButton
          ...
          :isLoading="lockBtn.transMoMoLock > 0"
          class="mgt-10"
          :onClick="recallAll"
        >
...
export default {
  ...
  computed: {
    ...
    momos() {
      return [...this.myNFT_verse].sort(
        (a, b) => (b.lvHashrate - a.lvHashrate) * 1
      );
    },
    getTotalSelectNum() {
  ...
  methods: {
    ...
    //
    async get1155() {
      let ids = Object.keys(BaseConfig.NftCfg).filter(
        (key) => BaseConfig.NftCfg[key].quality < 4
      );
    ...
    async get721() {
      ...
      this.momo_721 = await Wallet.ETH.momoStakeContract_verse.methods
    ...
    async recallAll() {
      ...
      Wallet.ETH.sendMethod(
        Wallet.ETH.momoStakeContract_verse.methods.recallAll(ids, amount),
        { from: address },
        (hash) => {
          if (hash) {

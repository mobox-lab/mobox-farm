<template>
  <div id="upgrade">
        ...
        <div class="panel">
          ...
          <div id="upgrade-power" class="vertical-children">
            <img
              ...
              class="tip-icon"
              v-if="!isMeetStandards && getNowPetItem.level <= 1"
              @click="standardsHashrateTip"
            />
            <div
              class="gka-harmer por dib"
              style="top: -8px; right: -10px"
              :class="
                this.getNowPetItem.location == 'wallet'
                  ? ''
                  : 'animation-harmer'
              "
            >
              <img
                src="@/assets/anime/sleep.gif"
                class="sleep-harmer"
                v-if="this.getNowPetItem.location == 'wallet'"
                alt=""
              />
            </div>
          ...
          <div class="por" id="show-pet-view" style="margin-top: 100px">
            ...
            <div
              class="vertical-children chanzi"
              v-if="this.getNowPetItem.location == 'verse'"
            >
              <img
                src="@/assets/icon/chanzi.gif"
                alt=""
                width="50"
                style="margin-bottom: -5px"
              />
              <span class="bold" style="font-size: 25px; color: #fff">{{
                getSCL
              }}</span>
            </div>
            <div class="vertical-children" id="upgrade-name">
              <img
                :src="require(`@/assets/icon/${this.getNowPetItem.chain}.png`)"
                height="25"
                alt=""
              />&nbsp;
              <span>{{
              ...
              <img
                v-if="
                  this.hasEditNameSkill && getNowPetItem.location == 'stake'
                "
                src="@/assets/icon/edit.png"
                class="edit-btn"
                alt=""
                @click="oprDialog('setName-dialog', 'block')"
              />
            </div>
          </div>
        </div>
      ...
      <div class="col-md-6 tal mgt-10">
        <div class="panel vertical-children">
          ...
          <div
            class="mgb-20"
            v-if="
              getNowPetItem.tokenId != 0 && getNowPetItem.location != 'wallet'
            "
          >
            <h3>
              {{ $t("MOMO_22") }}
              <img
                ...
                @click="getRootRefs().ruleDialog.show('MOMO_35', 'MOMO_36')"
            ...
            <div class="tac" style="margin-bottom: 20px">
              ...
              <div v-if="getUpgradeInfo.length != 0">
                <div style="margin-top: 30px">
                  <span
                    @click="showSelectNftDialog(item, handPos)"
                    v-for="(item, handPos) in this.getUpgradeInfo"
                ...
                <div class="vertical-children mgt-20" style="font-size: 18px">
                  ...
                  <span style="color: #85f34a">{{
                    this.getNowPetItemNextLvHashRate
                  }}</span>
                </div>
                <div>
                  <div class="mgt-20">
                    ...
                    <div :class="{ 'btn-group': needApproveMec }">
                      ...
                      <div class="mgt-10">
                        <StatuButton
                          ...
                          :isDisable="needApproveMec || lockUpgradeTime > 0"
                          :isLoading="lockBtn.enhanceLock > 0"
                        >
                          <Loading
                            v-if="lockUpgradeTime > 0"
                            class="btn-loading"
                          />
                          <svg
                            ...
                            width="15"
                            height="15"
                            style="transform: rotate(270deg); margin-top: -2px"
                          >
                            <path
                              fill="#ffffff"
    ...
    <Dialog id="setName-dialog" :top="200" :width="400">
      ...
      <button
        :class="`btn-primary mgt-20 ${!canSubmitEditName ? 'disable-btn' : ''}`"
        style="width: 200px"
    ...
    <Dialog id="selectNft-dialog" :top="30" :width="540">
      ...
      <div class="mgt-20 items">
        <span
          @click="showSelectNftDialog(item, handPos, true)"
          v-for="(item, handPos) in this.getUpgradeInfo"
          :key="JSON.stringify(item)"
        >
          <PetAddItem
      ...
      <p class="mgt-20 small tal por">
        <span
          class="type_change vertical-children"
          v-if="needHandleData.type != 'v5'"
        >
          <Tab
            :list="tab"
          ...
          <div
            class="dib sort-btn"
            @click="bookSortTurn = !bookSortTurn"
            :class="tab_pos == 0 ? 'opa-6 gray' : ''"
          >
      ...
      <div
        :class="tab_pos == 0 ? '' : 'hide'"
        class="dialog-content tal mgt-10"
        style="height: 400px"
      >
        <PetItemSmall
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      allowance_1155_to_721: (state) =>
        state.ethState.data.allowance_1155_to_721,
    ...
    showCanSelectArr() {
      ...
      this.myNFT_stake.map((item) => {
        ...
        if (
          "v" + item.vType == type &&
          (!needPrototype || needPrototype == item.prototype) &&
          !isSelf &&
          item.level >= needLv
        ) {
      ...
      [1, 2, 3, 4].map((item) => {
        ...
        if (needNum) {
          ...
          if (isHardAdvance) {
            ...
            reduceType = 4;
            reduceCategory = BaseConfig.NftCfg[hasSelectPetPrototype].category;
          }
          let targetPrototype =
            Number(reducePrototype) - (Number(reduceType) - item) * 10000;
          arr.push({
            type: "v" + item,
            needNum,
            needPrototype: isHard || isHardAdvance ? targetPrototype : false,
    ...
    canSubmitEditName() {
      return (
        this.inputName != "" && this.inputName != this.getNowPetItem.tokenName
      );
    },
    //
    getNftVInfo() {
    ...
    getBookArr() {
      ...
      Object.values(typeObj).map((item) => {
        ...
        if (itemArr.length == 1) {
          ...
          //
          let needType =
            !needPrototype || needPrototype % 10000 == pushArr[0].bookType;
  ...
  methods: {
    ...
    standardsHashrateTip() {
      this.getConfirmDialog().show(
        `${this.$t("MOMO_98")
          .replace("#0#", this.standardsHashrate)
          .replace("#0#", this.standardsHashrate)}`
      );
    },
    ...
    onTabChange(pos) {
      ...
      if (this.selectAddPetPos == 0) {
        this.showSelectNftDialog(
          this.getUpgradeInfo[0],
          this.selectAddPetPos,
          true
        );
      }
    },
    ...
    async approveNftToStake() {
      ...
      if (vType < 4) {
        await Wallet.ETH.approve1155ToTargetToken(WalletConfig.ETH.moMoStake);
      } else {
        await Wallet.ETH.approve721ToTargetToken(WalletConfig.ETH.moMoStake);
      }
    },
    //
    async stakeNft() {
      ...
      await Wallet.ETH.stakeNft(erc1155ids, erc1155Num, erc721TokenIds);
    },
    //
    onStepRemoveFood() {
        ...
        outLoop: for (let i = 0; i < hasArr.length; i++) {
          ...
          for (let j = 0; j < item.num; j++) {
            if (pushNum < needNum) {
              ...
              //
              if (item.vType > 3) {
                this.selectProtoTypes[type + "_tokenId"].push(item.tokenId);
              }
              pushNum++;
              hasOne = true;
            } else {
    ...
    async upgrade() {
      ...
      if (hash) {
        ...
        this.$store.commit("ethState/setUpgradeLocks", {
          tokenId,
          time: ConstantConfig.BTN_LOCK_MAX_TIME,
        });
        ...
        this.myNFT_stake.map((item, pos) => {
          ...
          } else if (needRemove721TokenArr.indexOf(item.tokenId) != -1) {
            this.myNFT_stake.splice(pos, 1);
          }
        });
        //
    ...
    async approved1155To721() {
      await Wallet.ETH.approve1155ToTargetToken(WalletConfig.ETH.moMoToken);
    },
    //
    change() {
      this.$store.commit("globalState/addNotify", {
    ...
    async confirmSetName() {
      ...
      this.hasClick = false;
      let res = await Wallet.ETH.setMomoNameByTokenId(
        tokenId,
        this.inputName,
        name == ""
      );
    ...
    showSelectNftDialog(needHandleData, index, isTabChange = false) {
      ...
      if (
        !isTabChange &&
        (this.getNowPetItem.level + 1) % 5 == 0 &&
        needHandleData.type != "v5"
      ) {
        let t = setTimeout(() => {
          clearTimeout(t);
          this.hasLoadBook = true;
    ...
    onSelectChange({ tokenId, prototype, num, vType }) {
      ...
      if (num > 0) {
        ...
        if (vType >= 4) {
          this.selectProtoTypes["v" + vType + "_tokenId"].push(tokenId);
        }
      } else {
        //
    ...
    async viewMECApproved() {
      const isApprove = await Wallet.ETH.isApprovedForAll(
    ...
    //
    async approveMEC() {
      const hash = await Wallet.ETH.approvedForAll(
        WalletConfig.ETH.crystalToken,
        WalletConfig.ETH.moMoToken,
        () => {
          this.viewMECApproved();

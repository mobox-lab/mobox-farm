<template>
  <div id="openbox" class="tac center-box">
        ...
        <div class="adv-panel por box-section" style="padding-bottom: 45px">
          ...
          <div class="por box" style="height: 287px; margin: 0px auto">
            <img
              ...
              alt=""
              id="show-rate"
              @click="oprDialog('show-rate-dialog', 'block')"
            />
            <div class="box-show">
              <div
                class="box-spine"
                ref="boxSpine"
                v-show="showOpenAnimation"
              ></div>
              <div
                class="box-spine"
                ref="boxAnimation"
          ...
          <div id="show-card-v2" class="hide" @click.stop="nextOpen">
            <div id="show-card-bg">
              <div
                :class="[
                  {
                    'animate__slideInUp animate__animated animate__faster':
                      !isOpenAll,
                  },
                ]"
                v-for="(item, pos) in petDataArr.slice(0, openNum - openPos)"
                :style="
                  !isOpenAll
                    ? `margin-top:-${
                        (openNum - openPos - pos) * 15
                      }px; transform: translate(-50%, -50%) scale(${
                        1.5 - (openNum - openPos - pos) * 0.02
                      }); `
                    : `transition-delay: ${60 * pos}ms; transform: translate(${
                        pointObj[openNum][pos].x
                      }%, ${pointObj[openNum][pos].y}%)`
                "
                :key="pos"
              >
                <template v-if="pos == openNum - openPos - 1 || !openOne">
                  ...
                  <div
                    ...
                    :data-pos="pos"
                    class="card"
                    :class="`card-v${anieVtype[item.vType]}`"
                  ></div>
                </template>
              </div>
            </div>
            <div
              id="show-card-btn"
              class="hide animate__fadeIn animate__animated"
              v-show="!isOpenAll && !openOne"
            >
              <button class="btn-primary" @click.stop="openOneCard">
                {{ $t("NewBOX_36") }}
              </button>
              <button class="btn-primary mgl-10" @click.stop="openAll">
                {{ $t("NewBOX_37") }}
              </button>
            </div>
          </div>
              ...
              <div class="aveage-box">
                <div class="tal">
                  <button
                    ...
                    @click="
                      setAction(23001);
                      oprDialog('get-box-dialog', 'block');
                      addKey =
                        parseInt(boxNum) > 200 ? 200 : parseInt(boxNum) || 0;
                    "
                  >
                    {{ $t("BOX_36") }}
                  </button>
    ...
    <div class="tab">
      <ul>
        <li
          :class="recordsTabIndex === 0 ? 'active' : null"
          @click="toggleRecordsTab(0)"
        >
          {{ $t("BOX_44") }}
        </li>
        <li
          :class="recordsTabIndex === 1 ? 'active' : null"
          @click="toggleRecordsTab(1)"
        >
          {{ $t("BOX_45") }}
        </li>
      </ul>
    ...
    <!-- 全网开箱记录 -->
    <div
      class="col-md-12 whole-records"
      style="padding: 0 10px 10px 10px"
      v-show="recordsTabIndex === 0"
    >
      <section
        ...
        <table class="small new-table" border="0" frame="void" rules="none">
          ...
          <tr v-for="(item, index) in wholeNetworkOpenBoxHistory" :key="index">
            <td class="avatar address">
              <div
                :class="['image', avatar[item.address] ? 'hover' : null]"
                @click="showAvatar(item.address)"
              >
                <img
                  :src="
                    avatar[item.address] ||
                    'https://img.momoworld.io/mini_avatar_ori_0.png'
                  "
                />
              </div>
            ...
            <td class="result">
              <div
                :class="['momo', `momo-type${item.momos[index].vType}`]"
                v-for="(_, index) in Math.min(5, item.momos.length)"
                :key="index"
              >
                <img
                  :src="
                    require(`@/assets/pet/${item.momos[index].prototype}.png`)
                  "
                  alt=""
                  width="90%"
                />
                <div class="type">
                  <img
                    :src="
                      require(`@/assets/icon/${
                        category_img[item.momos[index].category]
                      }.png`)
                    "
                    alt=""
                    width="15"
                ...
                <div class="preview-info">
                  <PetItem
                    :data="{ item: item.momos[index] }"
                    :class="
                      item.momos[index].vType >= 4 && !item.momos[index].noPrice
                        ? 'market'
                        : ''
                    "
                    class="no-search"
              ...
              <img
                @click="showWholeNetworkOpenBoxHistory(item)"
                height="25"
      ...
      >
        <table class="small new-table" border="0" frame="void" rules="none">
          ...
          <tr v-for="item in getOpenBoxHistory" :key="item.tx + item.event">
            ...
            <td class="tar">
              <img
                v-if="item.event == 'MintBox' && item.state == 1"
    ...
    <Dialog id="get-box-dialog" :top="200" :width="400">
      ...
      <div class="ly-input-content mgt-20">
        ...
        <div class="por mgt-5">
          ...
          <input
            ...
            v-int
            :data-max="parseInt(boxNum) > 200 ? 200 : parseInt(boxNum)"
            v-model="addKey"
          />
          <div class="dib" style="width: 30%">
            <button
              @click="
                addKey = parseInt(boxNum) > 200 ? 200 : parseInt(boxNum) || 0
              "
      ...
      <div
        :class="newBoxApproveToMinter == false ? 'btn-group' : ''"
        class="mgt-20"
      >
        <StatuButton
          ...
          data-step="1"
          style="width: 80%"
          v-if="newBoxApproveToMinter == false"
        ...
        <button
          ...
          :class="newBoxApproveToMinter == false ? 'disable-btn' : ''"
        >
          {{ $t("BOX_41") }}
        </button>
      </div>
      ...
      <div class="ly-input-content mgt-20">
        ...
        <div class="por mgt-5">
          ...
          <div class="dib" style="width: 30%">
            <button
              @click="
                openBox = canOpenBox > maxOpenOne ? maxOpenOne : canOpenBox || 0
              "
              class="btn-primary btn-small"
              style="width: 80%"
            >
    ...
    <div class="preview l" v-if="showPreview" @click="showPreview = false">
      <div
        class="preview-content adv-panel animate__zoomIn animate__animated animate__faster"
      >
        <h2>{{ $t("BOX_43") }}</h2>
        <div
          class="adv-panel-content mgt-10"
          style="padding: 20px"
          v-if="previewMomo.vType != ''"
        >
          <div class="vertical-children">
            <img
              :src="
                require(`@/assets/icon/${
                  category_img[previewMomo.category]
                }.png`)
              "
              alt=""
            />
          ...
          <div class="momo-show por dib mgt-10">
            ...
            <img
              class="momo-show-momo"
              :src="require(`@/assets/pet/${previewMomo.prototype}.png`)"
            />
          </div>
          <h2 :class="`c-lv${previewMomo.vType}`">
            {{ selectVType[previewMomo.vType] }}
          </h2>
          <div class="vertical-children">
            ...
            <span class="mgl-5"
              >{{
                previewMomo.vType > 3
                  ? getStaticHashPower[previewMomo.vType] - getRandomInt(0, 20)
                  : getStaticHashPower[previewMomo.vType]
              }}
              hash power</span
            >
          </div>
        ...
        <div class="mgt-20 tac">
          <button class="btn-primary" @click="showPreview = false">
            {{ $t("Network_12") }}
          </button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    getStaticHashPower() {
      console.log([
        ...
        2,
        3,
        this.momoSetting.v4_max_upgrade,
        this.momoSetting.v5_max_upgrade,
      ]);
      return [
        0,
        1,
      ...
      openBoxTemp.map((item, index) => {
        if (historyObj[item.tx] == undefined) {
          ...
          tokenIds.map((item, key) => {
            let { category, hashrate, prototype, quality, specialty } =
              tokens[(item, key)];
    ...
    needApprove() {
      return (
        this.ethState.allowance_box_to_minter != -1 &&
        Number(this.ethState.allowance_box_to_minter) < 100
      );
    },
    isOpening() {
      let isOpening = false;
  ...
  methods: {
    ...
    async getAvatarByAddress(addresss) {
      const res = await Wallet.ETH.avatarHelper.methods
        .getFirstAvatar(addresss)
        .call();
      ...
      addresss.forEach((item, index) => {
        ...
        if (id !== "0") {
          this.$set(
            this.avatar,
            item,
            `https://img.momoworld.io/mini_avatar_ori_${id}.png`
          );
        }
      });
    },
    ...
    async getWholeNetworkOpenBoxHistory() {
      try {
        ...
        this.wholeNetworkOpenBoxHistory = res.map((item) => {
          ...
          const token721 = item.tokenIds.map((item, key) => {
            const { category, hashrate, prototype, quality, specialty } =
              tokens[key];
      ...
      this.openBoxHistoryTimer = setTimeout(
        this.getWholeNetworkOpenBoxHistory.bind(this),
        10000
      );
    },
    ...
    async cardClick(e) {
      if ((!this.isOpenAll && !this.openOne) || $(e.target).hasClass("play"))
        return;
      ...
      //
      if (this.openAllNeedSleep < delayTs + 1000)
        this.openAllNeedSleep = delayTs + 1000;
    ...
    async isApprove() {
      ...
      let newBoxApproveToMinter = await Wallet.ETH.isApprovedForAll(
...
@keyframes flipX {
  ...
  100% {
    -webkit-transform: perspective(1000px) translateZ(0) rotateX(180deg)
      scale(1);

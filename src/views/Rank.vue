  ...
  <div class="rank tac" style="padding: 10px">
    ...
    >
      <table style="width: 100%" class="new-table">
        ...
        <tr
          v-for="item in hashrateRankList.list.slice(
            10 * (rewardPage - 1),
            10 * rewardPage
          )"
          :key="item.rank"
        ...
        <tr v-if="hashrateRankList.list.length > 10">
          <td colspan="2" style="border: none">
            <Page
              :defaultPage="1"
              :totalPage="Math.ceil(hashrateRankList.list.length / 10)"
        ...
        <tbody class="my-rank">
          <tr>
            ...
            >
              <span
                class="dib tac rank-icon"
                :class="{ active: hashrateRankList.self.rank <= 3 }"
                >{{ hashrateRankList.self.rank || "-" }}</span
              >
              <span style="margin-left: 20px">
                {{ shorAddress(hashrateRankList.self.address) }}
              </span>
            </td>
            <td
              class="tar vertical-children"
              style="
            ...
            >
              <span class="dib tal">
                {{ hashrateRankList.self.score || "-" }} </span
              >&nbsp;
              <img src="../assets/icon/hash_icon.png" alt="" height="30" />
            </td>
          </tr>
    ...
    >
      <table style="width: 100%" class="new-table">
        ...
        <tr
          v-for="item in momoRankList.list.slice(
            10 * (rewardPage - 1),
            10 * rewardPage
          )"
          :key="item.rank"
          ...
          <td class="tal vertical-children tac-xs">
            ...
            <div class="dib rank-pet-name">
              <img
                :src="
                  require(`../assets/icon/${
                    category_img[nftCfg[item.prototype]['category']]
                  }.png`)
                "
                alt=""
                height="16"
        ...
        <tr v-if="momoRankList.list.length > 1">
          <td colspan="4" style="border: none">
            ...
            <Page
              :defaultPage="1"
              :totalPage="Math.ceil(momoRankList.list.length / 10)"
              :onChange="onPageChange"
            />
          </td>
        </tr>

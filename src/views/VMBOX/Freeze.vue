      ...
      <div class="ly-input-content por">
        ...
        <div class="por mgt-5">
          ...
          <input
            class="ly-input dib"
            type="text"
            style="text-align: center; width: 100%; padding-left: 50px"
            v-number
            :data-max="getMaxMbox"
            data-min="1"
            v-model="inputNum"
            :placeholder="$t('Air-drop_156')"
          />
        ...
        <section class="mgt-10" id="select-time">
          ...
          <div class="ly-input-content aveage-box tac">
            ...
            <div v-for="day in [7, 15]" :key="day">
              <button
                class="btn-small"
                :class="selectDay == day ? 'btn-primary' : 'btn-default'"
                @click="selectDay = day"
              >
                {{ $t(freezeConf[day].lang) }}
              </button>
            </div>
            <div></div>
          </div>
          <div class="ly-input-content aveage-box tac">
            ...
            <div v-for="day in [30, 90, 180]" :key="day">
              <button
                class="btn-small"
                :class="selectDay == day ? 'btn-primary' : 'btn-default'"
                @click="selectDay = day"
          ...
          <div class="ly-input-content aveage-box tac">
            ...
            <div v-for="day in [365, 730, 1095]" :key="day">
              <button
                class="btn-small"
                :class="selectDay == day ? 'btn-primary' : 'btn-default'"
                @click="selectDay = day"
        ...
        <div class="tac mgt-10">
          <p v-if="Number(inputNum) > 0">
            {{ $t("Air-drop_166") }}:
            <span class="notice-color">{{ getCanFreezeVeMbox }}</span> veMBOX
          </p>
          <!-- <StatuButton class="mgt-10" :isDisable="Number(inputNum) <= 0" :isLoading="lockBtn.freezeMboxLock > 0" :onClick="freeze">确定冻结</StatuButton> -->

          <div
            :class="{ 'btn-group': coinArr['MBOX'].allowanceToVeMbox == 0 }"
            v-if="oprData.coinName == 'MBOX-BNB' || oprData.coinName == 'GOV'"
          >
            <StatuButton
              ...
              class="mgt-10"
              style="width: 70%"
              :onClick="approve.bind(this, 'MBOX')"
              v-if="coinArr['MBOX'].allowanceToVeMbox == 0"
              >{{ $t("Air-drop_16") }} MBOX</StatuButton
            >
            <StatuButton
              :isLoading="lockBtn.freezeMboxLock > 0"
              :isDisable="
                coinArr['MBOX'].allowanceToVeMbox <= 0 || Number(inputNum) <= 0
              "
              data-step="2"
              class="mgt-10"
              style="width: 70%"
          ...
          <p v-else class="color-buy">
            {{ $t("Air-drop_215") }}
            <span class="cur-point" @click="$root.$children[0].showNotice()"
              >{{ $t("Air-drop_216") }}>></span
            >
          </p>
        </div>
    ...
    <div class="tab-body mgt-10" v-if="oprData.coinKey != 'GOV'">
      <div class="tab-content tal">
        <section>
          ...
          >
            <table
              class="tal mgt-10 my-table small"
              style="width: 100%; margin: 10px auto; border-color: #19233e"
              border="0"
              cellpadding="0"
              cellspacing="0"
            >
              ...
              <tr>
                <td style="width: 55%">
                  {{ $t("Air-drop_169").replace("#0#", sliderValue + "x") }}:
                </td>
  ...
  computed: {
    ...
    getNeedVeMboxObj() {
      ...
      if (coinObj) {
        ...
        needObj.veMbox = this.numFloor(
          ((y * (veMoboxSupply - this.getTotalVeMbox * 1e18)) / (1 - y) -
            this.getTotalVeMbox * 1e18) /
            1e18,
          1e4
        );
        if (
          veMoboxSupply == 0 ||
          veMoboxSupply <= this.getTotalVeMbox * 1e18 ||
          myShare >= Number(shareTotal)
        ) {
          needObj.veMbox = 0.1;
        }
        //
    ...
    hasStakeCoin() {
      return !(
        this.oprData.coinKey != "" &&
        this.coinArr[this.oprData.coinKey].wantAmount <= 0
      );
    },
  },
  async created() {
  ...
  methods: {
    async setAllowance(coinKey) {
      if (
        coinKey != "" &&
        coinKey != "BNB" &&
        this.coinArr[coinKey].allowanceToVeMbox == -1
      ) {
    ...
    async approve(coinKey) {
      ...
      let hash = await Wallet.ETH.approveErcToTarget(
        PancakeConfig.SelectCoin[coinKey].addr,
        WalletConfig.ETH.momoVeMbox,
        { coinKey, type: "allowanceToVeMbox" }
      );
      if (hash) {
    ...
    async freeze() {
      ...
      if (
        orderIndexsCountDown[orderIndex_] <= 0 &&
        orderIndexs[orderIndex_].stakeMbox > 0
      ) {
      ...
      let obj = {
        poolIndex_:
          coinKey == "GOV" ? 100001 : PancakeConfig.StakeLP[coinKey].pIndex,
        amount_,

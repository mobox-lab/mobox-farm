  ...
  <section>
    <Dialog id="vmbox-dialog" :top="100" :width="420">
      ...
      <div class="tal mgt-10">
        ...
        <div
          @click="dialog_tab_pos = 2"
          :class="dialog_tab_pos == 2 ? 'active' : ''"
          class="tab-menu por"
        >
        ...
        <div class="pancake-setting">
          <div class="dib por">
            <svg
              @click="
                getLog();
                oprDialog('vembox-log-dialog', 'block');
              "
              viewBox="0 0 24 24"
...
export default {
  ...
  computed: {
    ...
    hasMboxToRetrieve() {
      ...
      for (let key in this.getVeMboxObj.orderIndexsCountDown) {
        if (
          this.getVeMboxObj.orderIndexsCountDown[key] == 0 &&
          this.getVeMboxObj.orderIndexs[key].stakeMbox > 0
        ) {
          hasToRetrieve = true;
        }
      }
  ...
  methods: {
    ...
    async getPoolVeMobox() {
      ...
      let pIndex =
        this.oprData.coinKey == "GOV"
          ? "100001"
          : PancakeConfig.StakeLP[this.oprData.coinKey].pIndex;
